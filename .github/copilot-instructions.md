...
- **Always use `Config.py` for passivbot configs**: Loading and saving any passivbot config (backtest, optimizer, live) MUST go through the appropriate Config class (`ConfigV7`, etc.). Never read or write config JSON files directly — always use `cfg = ConfigV7(path); cfg.load_config()` to load and `cfg.save_config()` to save. This ensures proper validation, normalization, and schema completeness.
