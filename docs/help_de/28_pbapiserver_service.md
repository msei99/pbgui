...
Fehlt bei einer bestehenden systemd-Installation nach dem Update noch `pbgui-vps-monitor.service`, führt der nächste Restart nach dem Laden des Migrationscodes eine einmalige Migration aus. PBGui installiert die Unit ohne den Zustand optionaler Dienste zu verändern, stoppt den alten API-eigenen Monitor, prüft den RPC-Endpunkt des Daemons und startet erst danach die API. Bei diesem ersten Handoff verbinden sich die vorhandenen SSH-Sessions einmal neu; spätere API-Restarts lassen sie verbunden. Wurde der Code manuell ersetzt, während noch der alte API-Prozess lief, lädt der erste Restart nur den neuen Code; der orange Hinweis bleibt für den zweiten, migrierenden Restart sichtbar.
