...
Die Navigationsleiste zeigt einen orangefarbenen **Restart**-Button, wenn der API-Prozess oder ein aktiver PBCluster-, PBRun-, PBData-, PBCoinData-, PBMonitorAgent- oder Vast-GPU-Pool-Prozess noch einen älteren Wert aus `api/serial.txt` verwendet. Der Bestätigungsdialog listet die betroffenen Dienste auf. Beim Neustart des Vast-GPU-Pools wird nur dessen lokaler Miet-Scheduler neu geladen; bestehende Vast-Mieten, Uploads und Optimizer-Prozesse laufen weiter. Detached Bots, Backtests, Optimierungen und Market-Data-Jobs werden nicht neu gestartet.
