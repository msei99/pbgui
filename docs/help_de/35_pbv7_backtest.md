...
Auch die Configs-Tabelle bietet **V8** fuer die gespeicherte V7-Backtest-Config. Beide Konvertierungen lassen die V7-Quelle unveraendert und uebergeben die vollstaendige Config an PB8. Bei der Konvertierung eines Results ermittelt PBGui vor der Migration die effektiv verwendeten Maker- und Taker-Raten aus dessen linearen `fills.csv`-Daten. Vor der Migration laesst PBGui retired nur fuer Live-Ausfuehrung relevante Felder weg, kanonisiert `backtest.aggregate` zu `backtest.reducer` und entfernt einen doppelten singulaeren Exchange, wenn er bereits `backtest.exchanges` entspricht. Das Backtest-Review blendet Live- und Optimize-Befunde aus, behaelt aber Strategy-Bot-Befunde bei, weil sie das simulierte Ergebnis beeinflussen. Bleiben echte kontextrelevante Felder offen, veroeffentlicht PBGui keine lauffaehige Config und oeffnet kanonischen Review-Output als ungespeicherten PB8-Backtest-Editor-Draft mit roten Markierungen an verfuegbaren strukturierten Feldern. Ungueltiger Output bleibt blockiert. Eine wiederholte Konvertierung mit bereits vorhandenem Ziel oeffnet diese V8-Config ohne zusaetzlichen Bestaetigungsdialog direkt und ueberschreibt sie nicht.
