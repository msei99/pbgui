...
| **Import Config** | JSON aus Text, einer lokalen Datei oder einem Backtest Archive importieren |
| **Add to Archive** | Ausgewählte Configs im PB7-versionierten Layout nach **My Archive** exportieren |
| **Queue Selected** | Alle ausgewählten Configs zur Queue hinzufügen |
| **Delete Selected** | Alle ausgewählten Configs löschen |

Archiv-Importe werden als lokale Optimize-Configs gespeichert, bevor der Editor geöffnet wird. Existiert der gewünschte lokale Name bereits, bietet PBGui **Overwrite**, **Import as Copy** und **Cancel** an; die Copy-Option wählt einen freien Namen und passt `backtest.base_dir` entsprechend an. Die archivierte Quelle bleibt dabei unverändert, auch in read-only Community-Archiven.

**Add to Archive** ist nur verfügbar, wenn auf der Backtest-Seite ein beschreibbares **My Archive** konfiguriert ist. PBGui erhält oder ergänzt `config_version`, migriert ein sauberes eigenes Legacy-Archiv vor dem Export und verwendet bei identischem Inhalt die bestehende archivierte Config samt ursprünglichen Metadaten statt weiterer nummerierter Kopien.
...
| **🔎 Apply Filters** | Wendet `market_cap`, `vol/mcap`, Tags, `only_cpt` und `notices_ignore` auf alle gewaehlten Exchanges an und ersetzt beide Long-/Short-Listen fuer Approved und Ignored durch das vereinigte Ergebnis |
...
Filterwerte sind nur Config-Metadaten, bis **Apply Filters** geklickt wird. Reines Speichern erhaelt die Zahlen, berechnet aber die expliziten Approved-/Ignored-Listen nicht neu.
...
Der Bereich **Suite Mode** verwendet jetzt denselben gemeinsamen Editor wie der FastAPI-Backtest, statt Suite-Configs nur in einer Legacy-Optimize-Seite bearbeitbar zu lassen. Dadurch lassen sich `backtest.suite_enabled`, `backtest.scenarios` und `backtest.aggregate` direkt im FastAPI-Optimize-Editor bearbeiten und speichern, einschließlich der eingebauten Templates, Szenario-Overrides, per-Szenario-`coin_sources` und Aggregationsregeln. Aktive Szenariofelder bleiben beim Hinzufuegen, Verschieben oder Entfernen von Szenarien sowie beim Deaktivieren von Suite Mode automatisch erhalten; **Done** ist davor nicht erforderlich.
...
| **crossover_probability + mutation_probability** | Der DEAP-Editor hält die kombinierte Wahrscheinlichkeit bei höchstens `1.0` und entspricht damit wieder dem alten Optimize-Verhalten |
...
| **Settings** | Öffnet den Queue-Settings-Dialog für `Autostart`, den Queue-CPU-Wert und die Launch-Option `Use PBGui Market Data` |
...
Bei Suite-Results folgen die **Summary**-Badges jetzt der Toolbar-Auswahl, statt immer nur eine feste Aggregatansicht zu zeigen. Damit lassen sich direkt in der FastAPI-Seite entweder die aggregierte Suite-Statistik oder einzelne Szenarien prüfen, ohne in eine Legacy-Optimize-Seite zu wechseln.
