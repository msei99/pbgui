...
1. **Settings (Binance USDM Latest 1m Auto-Refresh)** öffnen → Filter leeren → **Select visible** → **Save**
2. **Settings (Latest 1m Auto-Refresh) — Hyperliquid** öffnen → Filter leeren → **Select visible** → **Save**
3. Exchange-Dropdown auf **Bybit** umschalten → **Settings (Bybit Latest 1m Auto-Refresh)** öffnen → Filter leeren → **Select visible** → **Save**
4. Für **OKX** und **Bitget** wiederholen, wenn deren lokale 1m-Datensätze verwendet werden.
...
Antworten der Settings-API enthalten additiv ein `apply`-Objekt mit `timing`,
`restart_required` und einer sicheren `message`. Latest-1m-Einstellungen
gelten im nächsten PBData-Zyklus; Archivpfade und TradFi-Profile werden bei der
nächsten Operation gelesen. Daraus folgt weder ein globaler Market-Data-Watcher
noch ein erforderlicher Dienstneustart.
...
| **Bybit** | CCXT (async) | ~3 Stunden (~550 Coins) |
| **OKX** | Offizielle Archiv-ZIPs plus REST-Reparatur | abhängig von ausgewählten Coins und Historie |
| **Bitget** | Nur REST `USDT-FUTURES`, optional verteilt | ~7–9 Minuten für die vollständige BTC-Historie auf einem Downloader |
| **Hyperliquid** (Crypto) | l2Book-Archiv + 1m\_api-Konvertierung | abhängig von der l2Book-Archivgröße |
...
- Bitget BTC ab Inception (nur REST, gemessen): **~8 min 39 s** für 3,66 Millionen Candles; die Laufzeit für mehrere Coins hängt von Auswahl und optionalen Downloadern ab.
...
- OKX: letzte **2–7 Tage** werden alle 3 600 s (1 h) pro Zyklus aktualisiert
- Bitget: letzte **2–7 Tage** werden per öffentlichem REST alle 3 600 s (1 h) pro Zyklus aktualisiert
...
1. Settings (Latest 1m Auto-Refresh) für die gewählte Exchange
2. Market Data Status für die gewählte Exchange
3. Build best 1m OHLCV
4. TradFi Symbol Mappings
5. Download l2Book from AWS (nur Hyperliquid)
...
- `TradFi / Tiingo` — serverseitiger Tiingo-Profilstatus und TradFi-Mapping-Steuerung
...
Hyperliquid `Best 1m` folgt jetzt auch den neueren FastAPI-Editor-Mustern: Für den Build-Zeitraum wird derselbe Popup-Kalender wie in den Backtest-/Optimize-Editoren verwendet, und die Coin-Auswahl erscheint als mehrspaltiges Enabled-Coins-Grid mit `Filter enabled coin list`, `TradFi only`, `No downloaded history`, `Select visible` und `Clear all` statt des alten kompakten Dropdowns. XYZ-Build-Optionen sind auf kanonische Mappings mit nutzbarem Tiingo- oder Tiingo-FX-Ticker begrenzt; `pending`, `no_provider` und `delisted` werden nicht angeboten, weil der Downloader sie nicht laden kann. Bei XYZ-Coins behält `No downloaded history` Symbole ohne eine einzige Tiingo-gestützte `other_exchange`-Minute im lokalen Source-Index. Ein einzelner aktueller Hyperliquid-Tag blendet einen Coin, dessen historischer Backfill noch fehlt, dadurch nicht aus. Beide Schalter lassen sich mit dem Textfilter kombinieren. Die sichtbaren Coin-Zeilen sind jetzt direkt anklickbar und unterstützen zusätzlich Maus-Drag-Selektion, damit größere Bereiche ohne Checkbox-Klicks markiert oder entfernt werden können. Auch schnelle Drag-Bewegungen interpolieren jetzt die Zwischenzeilen, sodass beim zügigen Durchziehen keine Coins mehr übersprungen werden.
...
Für Binance, Bybit, OKX und Bitget nutzt die Coin-Auswahl direkt ein Settings-ähnliches Available-Coins-Grid im FastAPI-Panel: `Filter available coin list` schränkt das Grid ein, `Select visible` übernimmt den aktuell gefilterten Ausschnitt, `Clear all` setzt die explizite Auswahl zurück, und du kannst mit der Maus über sichtbare Coin-Zeilen ziehen, um größere Bereiche schnell zu markieren oder wieder zu entfernen. Schnelle Drag-Bewegungen füllen auch die Zwischenzeilen. Wenn die Auswahl leer bleibt, werden alle verfügbaren Coins eingereiht; eine explizite Auswahl begrenzt den `Best 1m`-Job auf genau diese Coins.

Diese FastAPI-`Best 1m`-Ansicht startet für Binance, Bybit, OKX und Bitget direkt mit den eigentlichen Build-Feldern. Der redundante Einleitungskopf mit Text und der zusätzliche obere `Refresh`-Button wurden entfernt.

Für Binance, Bybit, OKX und Bitget zeigt das FastAPI-`Best 1m`-Build-Panel direkt unter dem vollständigen Build-Formular den gefilterten Job Monitor an. Damit bleiben eingereihte, laufende, erfolgreiche und fehlgeschlagene Jobs der gewählten Exchange sichtbar, ohne in das separate Activity-Log-Panel wechseln zu müssen.
...
Beide Monitor-Varianten rendern Job-Payloads, Fortschrittswerte und Backend-Fehler strikt als Text, waehrend Karten, Buttons, Dialoge und Expander unveraendert bleiben. Unerwartete Werte aus gespeicherten Jobs oder externen Fehlermeldungen koennen dadurch nicht als ausfuehrbares Seiten-Markup interpretiert werden. Im gemeinsamen Monitor bleiben Run- und Cancel-Aktionen sichtbar in Bearbeitung und koennen bis zum Eintreffen eines frischen Jobstatus nicht doppelt gesendet werden. Auch das massenhafte Loeschen laeuft nur einmal gleichzeitig und aktualisiert den betroffenen Tab vor der Erfolgsmeldung.
...
Das FastAPI-Panel `OHLCV Data` bündelt den kompletten Datenprüfungsablauf an einer Stelle. Für die gewählte Exchange erscheinen die Dataset-Buttons direkt in der Sidebar: `1m`, `PB7 cache` und das getrennte `PB8 cache`-Inventar sind immer verfügbar, auf Hyperliquid zusätzlich `1m_api` und `l2Book`. Im Hauptbereich erscheinen Summary-Metriken, eine filterbare Inventory-Tabelle, Delete-Tools für schreibbare Datasets, eine Coverage-Heatmap, bei Verfügbarkeit eine Minute-Heatmap und optional ein OHLCV-Detailchart. Beide Runtime-Cache-Inventare sind read-only und verwenden getrennte SQLite-Inventarschlüssel, auch wenn Exchange, Timeframe und Coin identisch sind.
...
In `PB7 cache` und `PB8 cache` gibt es oberhalb der Tabelle einen kleinen Timeframe-Schnellfilter direkt neben `Select All` und `Deselect`. Damit kannst du vor der Auswahl zwischen `all`, `1m` und `1h` umschalten, sodass Short-Name-Dubletten wie `ADA` getrennt nach Cache-Timeframe eingegrenzt werden können.
...
- **Filter leeren + Select visible / Clear all** — alle Coins schnell aktivieren oder deaktivieren
...
Multiplier-präfixierte Kontrakte behalten ihren kurzen Coin-Namen, wenn sie der einzige Markt für diesen Basiswert sind; `1000SHIB` bleibt beispielsweise `SHIB`. Listet dieselbe Exchange für dieselbe Quote beide Kontrakte, trennt PBGui sie: `1000CAT` steht auf Binance und Bitget für Simon's Cat, `CAT` für den unpräfixierten Caterpillar-Markt. Vorhandene Market-Data-Auswahlen werden vom alten mehrdeutigen Namen `CAT` auf `1000CAT` migriert. `CAT` muss separat gewählt werden, wenn auch der unpräfixierte Markt automatisch aktualisiert oder per Best 1m gebaut werden soll.
...
Der automatische Bybit-Zyklus aktualisiert nur den laufenden UTC-Tag. Beim ersten Zyklus ab `00:15 UTC` finalisiert PBData den Vortag für jeden aktivierten Coin. Ein erfolgreiches Ergebnis wird in `data/ohlcv/checksums.sqlite` gespeichert und bleibt über Neustarts hinweg erhalten; nur fehlgeschlagene Coins werden in späteren Stundenzyklen erneut versucht.
...
Bei Bybit trennt der Status den Current-Day-Refresh außerdem mit `current_result` und `current_minutes_written` von der täglichen Finalisierung mit `finalization_day`, `finalization_result` und `last_finalized_day`.

### Verhalten nach Neustart

Nach einem Neustart beginnt PBData nach seinem normalen Start-Offset einen neuen Zyklus. Erfolgreiche Bybit-Finalisierungen werden anhand des persistenten Checksum-Katalogs übersprungen; fehlgeschlagene oder unterbrochene Coins bleiben für den nächsten Stundenversuch verfügbar.

## OHLCV Integrity

Unter **OHLCV Integrity** wird der tägliche Checksum-Katalog für die oben in Market Data gewählte Exchange angezeigt. Read-only Scan, Summary, gruppierte Findings, Checksummen und öffentlicher Referenzvergleich unterstützen Binance USDM (Storage `binanceusdm`), Bybit, OKX, Bitget und Hyperliquid Crypto. Hyperliquid-XYZ-/TradFi-Verzeichnisse bleiben ausgeschlossen, bis eine session-aware Validierung umgesetzt ist.

Der erste Bybit-Scan wird nach einem Update automatisch eingereiht. Scans für Binance USDM, OKX, Bitget und Hyperliquid Crypto bleiben in diesem ersten Rollout bewusst manuell, weil das gemeinsame Archive ungefähr 1,8 Millionen Tagesdateien enthält. Exchange wählen und **Run full scan** verwenden; Integrity-Scans laufen serialisiert. Bis zum Abschluss zeigt die Exchange `Pending`. Nicht-Bybit-Kataloge sind Point-in-Time-Snapshots und müssen nach Änderungen ihrer Dateien erneut gescannt werden.

Wiederholte Full Scans übernehmen Katalogergebnisse für Dateien, deren nanosekundengenauer Änderungszeitpunkt und Größe unverändert sind. Die Dateien werden weiterhin entdeckt, damit gelöschte oder neu hinzugekommene Tage sichtbar bleiben, aber ihr NPZ-Inhalt wird nicht erneut geöffnet oder gehasht. Geänderte Dateien sowie Tage mit möglicherweise veränderter Inception-/Current-Day-Klassifizierung werden immer neu validiert.

Eine zusammenhängende Suffix-Datei am jeweils frühesten lokalen Coin-Tag wird als `inception_partial` klassifiziert; vor der ersten lokalen Datei erzeugt der Scanner keine fehlenden Tage. Spätere partielle Tage und interne Lücken bleiben ungültig. Ein neuester partieller abgeschlossener Tag wird nicht allein aufgrund seiner lokalen Position akzeptiert.

Findings werden pro Coin mit Anzahl und Zeitraum der beschädigten Tage, fehlenden Minuten und Prüfgründen gruppiert. Alle fünf Exchanges bieten **Repair coin** und **Repair all**. Bybit verwendet seinen Exact-Day-Finalizer; Binance USDM, OKX und Bitget fetchen über ihre normalen Builder ausschließlich den angeforderten beschädigten Tag neu. Die Hyperliquid-Reparatur verbessert genau diesen Tag mit verfügbaren Hyperliquid-API-/L2Book-Daten und füllt verbleibende Minuten danach zuerst mit Binance und anschließend Bybit. Jeder Pfad berechnet die Checksumme neu und lässt ein Finding bestehen, wenn das Ergebnis weiterhin unvollständig ist.

Integrity validiert die von Passivbot verwendeten Werte High, Low, Close und Volume. Exchange-Open-Werte außerhalb von High/Low werden akzeptiert, weil Open nicht in Passivbots Backtest-Repräsentation gespeichert wird. Für Hyperliquid bietet **Normalize fallback candles** einen bestätigten Maintenance-Job für historische `other_exchange`-Minuten aus dem Cross-Exchange Fallback. Der Job erweitert ausschließlich High/Low um die vorhandenen Open-/Close-Werte; Timestamps, Open, Close, Volume, Gaps und Source-Zuordnungen bleiben unverändert. Neu erzeugte Fallback-Candles werden automatisch normalisiert.

Die Tabelle lädt immer alle aktuell beschädigten Tage. **Repair all** verarbeitet sie nacheinander in einem abbrechbaren Hintergrundjob. Ein Fehler bei einem Tag bleibt im Ergebnis sichtbar und stoppt die übrigen Reparaturen nicht. Summary und Zeilen aktualisieren sich automatisch, sobald ein Integrity-Job abgeschlossen ist.

Die Repair Queue verwendet eine Zeile pro Coin, statt denselben Coin für jeden beschädigten Tag zu wiederholen. Jede Zeile fasst Anzahl und Zeitraum der beschädigten Tage, insgesamt fehlende Minuten und die Reason-Anzahlen zusammen. **Repair coin** startet einen sequenziellen Batch für alle beschädigten Tage genau dieses Coins.

Mit **Details** kann jeder beschädigte Tag vor der Reparatur geprüft werden. Die Minute-Coverage zeigt 24 Stundenzeilen mit einer Zelle pro Minute: vorhandene Candles sind grün, ein führender Bereich am frühesten lokalen Tag wird gelb als mögliche Exchange-Inception markiert, interne Gaps sind rot und nachlaufende Gaps orange. Bei mehreren Findings des gruppierten Coins steht eine Tagesauswahl bereit; die Range-Tabelle nennt exakten UTC-Start, UTC-Ende und Minutenanzahl.

Oberhalb des Minutencharts zeigt **Surrounding Days** sieben Tage vor und sieben Tage nach dem gewählten Datum als kompakte 24-Stunden-Coverage. Vollständige Stunden sind grün, partielle Stunden orange und fehlende Stunden rot. Jeder katalogisierte Nachbartag kann angeklickt und mit vollständiger Minutenansicht geprüft werden, auch wenn er gültig ist und nicht in der Repair Queue steht.

Die **Missing**-Anzahl der Repair Queue schließt einen führenden Bereich am frühesten lokalen Coin-Tag aus, weil der Handel lediglich später als `00:00 UTC` begonnen haben kann. Die Detailansicht zeigt diese nicht vorhandenen Minuten weiterhin gelb. Als beschädigt zählen nur fehlende Minuten von der ersten verfügbaren Candle bis `23:59 UTC`.

Repair All wiederholt vorübergehende Netzwerk-/Timeout-Fehler einmal. Bybit prüft die Exchange-Inception unabhängig anhand der `launchTime` des exakten aktuellen Instruments, wenn beschädigte lokale Dateien die Erkennung verdecken würden. Binance führt die entsprechende Prüfung anhand der `onboardDate` des exakten aktuellen Markts durch, weil Binance ein Symbol wiederverwenden und trotzdem die vorherige Instrumentgeneration weiterhin über Archiv und API ausliefern kann. Bestätigt eine der beiden Exchanges eine neuere Inception, entfernen **Repair coin** und **Repair all** automatisch die komplette veraltete lokale Generation vor diesem Datum einschließlich ihrer Source-Index-Abdeckung und Checksum-Zeilen; alle Tage ab der bestätigten Inception bleiben erhalten. Künftige Binance Full Builds starten ebenfalls erst ab dieser aktuellen `onboardDate`, damit die alte Generation nicht erneut geladen wird. Das Job-Ergebnis weist entfernte Tage separat aus. Am Starttag der Binance-BTC-USDT-Futures um `2019-09-08 19:00 UTC` und bei Bybit XTZ um `2021-01-11 05:50 UTC` gibt es jeweils eine unabhängig bestätigte native Quellenlücke; PBGui erfasst nur diese exakten Minuten als `source_gap`, statt Candles zu erfinden oder wiederholt unmögliche Reparaturen zu versuchen. Bleiben Fehler übrig, bleiben erfolgreiche Reparaturen gespeichert, aber der Batch erscheint mit seinem Teilergebnis unter **Failed** statt fälschlich vollständig erfolgreich unter Done.

Die Hyperliquid-Exact-Day-Reparatur prüft zuerst lokale sowie konfigurierte archivierte/NAS-L2Book-Stunden und führt danach den normalen Binance- und anschließend Bybit-Fallback aus. Vor der abschließenden Validierung erweitert sie bei vorhandenen historischen `other_exchange`-Candles dieses exakten Tags außerdem High/Low um Open/Close. Bleibt der historische Tag unvollständig, vergleicht PBGui jede exakt fehlende Minute mit den lokalen `onboardDate`-/`launchTime`-Metadaten der aktuellen Donor-Instrumente. Minuten, die nachweislich vor beiden Donors liegen, einschließlich nativer PURR-Historie ohne passenden Donor, werden als `source_gap` katalogisiert. Laut Metadaten möglicherweise durch einen Donor abdeckbare Minuten werden erst akzeptiert, nachdem die exakten Tagesabfragen bei beiden Donors erfolgreich abgeschlossen wurden und die Candles trotzdem nicht liefern; ein Netzwerk- oder Exchange-Fehler hält den Tag beschädigt und reparierbar. Source-Gap-Zeilen verlassen die Repair Queue, bleiben aber im separaten Summary-Zähler **Source gaps** sichtbar. Der Status ist an den unveränderten NPZ-Fingerprint beziehungsweise das fortgesetzte Fehlen eines vollständigen Tags gebunden; neue oder geänderte Daten werden automatisch wieder streng validiert. PBGui erzeugt für diese Klassifizierung keine Carry-forward-Candles.

Die Tabelle **Unavailable Coin Data** folgt der gewählten Exchange und listet jeden lokalen Coin auf, der in ihrer aktuellen `mapping.json` fehlt oder inaktiv ist, auch wenn seine vorhandenen Dateien noch gültig sind. **From** und **To** zeigen die älteste und neueste lokale OHLCV-Tagesdatei des jeweiligen Markts. Zeilen können angeklickt oder per Click-Drag als Bereich markiert und danach mit **Remove selected** oder der `Delete`-Taste entfernt werden. **Remove all** erfasst alle aktuell sicher entfernbaren unavailable Markets. Eine serverseitige Vorschau validiert die gesamte Auswahl erneut und zeigt Marktanzahl, Dateien, Größe und Zeitraum vor der verpflichtenden Bestätigung. Ein einzelner persistenter Batchjob prüft danach jeden Markt unmittelbar vor der Löschung seiner vollständigen PBGui-OHLCV-Rohdaten und seines Source-Index erneut; ein einzelner Fehler überspringt keine späteren Märkte und bleibt im Job-Ergebnis sichtbar. Unsichere Zeilen werden von Remove all ausgeschlossen. Removed-Markets werden aus der Repair Queue ausgeblendet. PB7- und PB8-Runtime-Caches werden nicht entfernt. **Repair all** überspringt diese Märkte und weist ihre Anzahl separat aus.

Die Checksum-Freigabe verwendet die konfigurierten Config-/Optimize-Archive:

- **Publish checksum snapshot** aktiviert eine tägliche Veröffentlichung nach erfolgreicher Bybit-Finalisierung und abgeschlossenen Scans aller fünf Exchanges.
- **Publish archive** akzeptiert nur das konfigurierte beschreibbare eigene GitHub-Archive mit serverseitigem Zugriffstoken.
- **Reference archive** kann unabhängig ein beliebiges konfiguriertes öffentliches GitHub-Archive auswählen. Systeme, die nur vergleichen, benötigen weder Token noch Cluster-Mitgliedschaft.
- **Publish now** verlangt abgeschlossene Scans aller fünf Exchanges, erzeugt ein konsistentes SQLite-Backup, komprimiert es als `checksums.sqlite.gz` und ersetzt das Asset im festen GitHub-Release `checksums-latest`.
- **Refresh reference** lädt dieses Release-Asset anonym, validiert es, behält bei Fehlern die letzte gültige Kopie und vergleicht es read-only mit dem lokalen Katalog.

Der Checksum-Snapshot enthält nur OHLCV-Identifikatoren, Tageszähler, Prüfzustände, Zeitstempel und Inhalts-Hashes. Archive-Zugangsdaten bleiben serverseitig und werden weder in Market-Data-Task-Payloads, Release-URLs noch Logs aufgenommen. Das gewählte Referenz-Repository muss für anonyme Downloads öffentlich sein.
...
Auf der FastAPI-Seite verwenden Binance USDM, Bybit, OKX und Bitget direkt ein Settings-ähnliches Available-Coins-Grid im `Best 1m`-Build-Panel. Du kannst die Liste über `Filter available coin list` eingrenzen, einzelne sichtbare Zeilen anklicken, mit der Maus über sichtbare Zeilen ziehen, um größere Bereiche schnell zu markieren oder zu entfernen, oder mit `Select visible` den aktuell gefilterten Ausschnitt gesammelt übernehmen. Wenn keine explizite Auswahl gesetzt ist, reiht PBGui alle verfügbaren Coins der aktuellen Exchange ein.
...
- Coin-Auswahl aus allen verfügbaren Binance Coins
...
**`bybit_best_1m`** — historischer Bybit-REST-/CCXT-Backfill für verfügbare Coins.

**`okx_best_1m`** — OKX-Archiv-Backfill mit REST-Reparatur für fehlende Candles und Volumes.

**`bitget_best_1m`** — Bitget USDT-FUTURES REST-only historischer Backfill:
- Verwendet Bitget-Symbole aus `data/coindata/bitget/mapping.json` und schreibt `data/ohlcv/bitget/1m/<COIN_DIR>/YYYY-MM-DD.npz`.
- Ein lokaler, nicht verteilter Backfill validiert vollständige historische Tage gegen 1.440 Minuten; Listing-Tag und aktueller UTC-Tag dürfen partiell bleiben.
- Optional verteilt **Distributed download** fehlende Datumsbereiche auf gewählte VPS-Downloader und/oder den Master. Remote-Downloader streamen rohe Candles zurück; nur der Master schreibt NPZ- und Source-Index-Dateien. Spätere Läufe planen unvollständige mittlere historische Tage auch bei ausgeschaltetem `Refetch` erneut; Listing-/Range-Start-Tag und aktueller UTC-Grenztag bleiben Teil-Tag-Ausnahmen.
...
3. Tiingo-Vault-Profil und Symbol-Mapping-Status prüfen
