...
Die Zeilenaktionen **P**, **G** und **T** setzen nach einer expliziten Bestaetigung global `panic`, `graceful_stop` oder `tp_only` fuer Long- und Short-PB8-Positionen. Status zeigt den konfigurierten Globalmodus getrennt von der Prozesssynchronisierung. Sobald eine Seite erzwungen ist, leert **N** beide globalen Modi ueber eine versionsgebundene Bestaetigung; neue Entries koennen erst fortgesetzt werden, nachdem das Ziel die veroeffentlichte Config angewendet hat. Deaktivierte Seiten bleiben deaktiviert, Coin- und HSL-Overrides bleiben unveraendert und bereits ausgefuehrte Panic-Closes koennen nicht rueckgaengig gemacht werden. Jede Aktion verwendet die normale PB8-Bundle-Pipeline: Sie erstellt ein vollstaendiges Backup, erhoeht die Config-Version, validiert Config und Sparse Overrides durch PB8, veroeffentlicht die Cluster-Operation und versucht die sofortige Aktivierung auf dem Zielhost. Der Editor bildet PB8s kanonische Forced-Mode-Werte verlustfrei auf die sichtbaren Selektoren und zurueck ab.
...
- Backtest-Uebergaben normalisieren alte nullartige Verzeichniswerte wie `None` auf PB8s kanonische Defaults `logging.dir = logs` und `monitor.root_dir = monitor`, damit neue Live-Instanzen nicht in versehentliche `None`-Verzeichnisse schreiben.
...
Import, Copy, Backtest-Uebergabe, **Strategy Explorer**, Live-Logs und Raw-JSON-Bearbeitung stehen im gleichen Sidebar-Ablauf bereit. PBGui AI kann fuer einen exakten aktiven Bot aus der Run-Liste oder von einer anderen Seite die angebotene Aktion `show_log` ausfuehren; PBGui behaelt die Aktion waehrend der Navigation ueber den Bot-Editor offen und verwendet danach die bereits vorhandene Live-Log-Funktion der Sidebar. Fuer einen aktiven PB8-Bot verwendet **Restart** eine eigene authentifizierte Backend-Aktion auf Basis der persistierten Host-Zuweisung; PBRun startet die gewuenschte Instanz erneut, waehrend der bestehende Log-Stream verbunden bleibt. Fuer gestoppte oder verschobene PB8-Bots ermittelt das Log-Panel den zuletzt beobachteten Host und die Config-Version, versucht diesen Host ueber den vorhandenen authentifizierten Log-Streamer und faellt danach auf mitgesicherte Backup-Logs zurueck. Kein roher Backup-Pfad gelangt in den Browser; der gemeinsame Viewer verwendet einen serverseitig allowlist-validierten PB8-Backup-Bezeichner. Strategy Explorer erhaelt die aktuelle ungespeicherte PB8-Config und alle referenzierten Sparse Overrides ueber einen authentifizierten opaken Draft. Der Import-Dialog bietet durchsuchbare User-Vorschlaege und weist Namen ausserhalb des konfigurierten Exchange-User-Katalogs ab. **Balance Calculator** oeffnet den gemeinsamen Rechner mit der aktuellen ungespeicherten Config; **Calc Balance** berechnet die Empfehlung direkt und kann sie als `balance_override` uebernehmen. Browser-Anfragen verwenden das HttpOnly-PBGui-Session-Cookie; der Editor rendert kein Session-Token.
...
PBv8 Run verwendet denselben **Backups**-Ablauf wie PBv7. Bevor eine bestehende Instanz ueberschrieben oder geloescht wird, speichert PBGui das vollstaendige vorherige Bundle unter `data/backup/v8`: `config.json`, alle referenzierten Sparse-Override-Dateien und jedes verfuegbare Supervisor- oder PB8-Runtime-Log. Die Retention-Einstellung bestimmt, wie viele Versionen pro Instanz erhalten bleiben.

**Load in editor** erzeugt einen kurzlebigen Draft zur Pruefung und zum normalen Speichern. **Rollback** ist fuer eine bestehende Instanz verfuegbar und verlangt, dass der Bot lokal sowie auf jedem relevanten Remote-Host nachweislich gestoppt ist. Die Aktion sichert das aktuelle Bundle, stellt die gewaehlte `config.json` und alle referenzierten Sparse Overrides wieder her, veroeffentlicht eine neue monotone Cluster-Version und laesst die wiederhergestellte Instanz deaktiviert. Fehlende, stale oder mehrdeutige Runtime-Beobachtungen lehnen den Rollback mit HTTP 409 ab, statt zu raten. Das Loeschen eines Backups betrifft nur dieses unveraenderliche Backup-Bundle.
