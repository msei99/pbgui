...
| **Check Linux Updates** | Den Paketstatus auf den ausgewaehlten VPS-Zeilen parallel neu pruefen und jedes erfolgreiche Ergebnis sofort veroeffentlichen |
...
Die Detailkoepfe fuer Master und VPS wiederholen die Versionskarten fuer PBGui, PB7 und PB8 samt Branch/Commit und Update-Status. Installierte Host-Daten stammen aus demselben Monitor-Agent-Snapshot wie die Overview-Zeile. Upstream-Branch-Heads kommen aus dem persistenten VPSMonitor-Release-Cache, sodass das Rendern der Seite keine Git- oder Netzwerk-Probes ausfuehrt.

Wenn eine PB8-Installation oder ein Update vor der Validierung endet, bleibt der Checkout im VPS Manager sichtbar, wird aber als nicht verfuegbar markiert. Die Overview-Zelle zeigt eine rote Warnung, die Detailansicht meldet **PB8 Update Required** mit dem gespeicherten Grund, und die dedizierte Aktion **Update PB8** bleibt hervorgehoben, bis ein erfolgreiches Update den Runtime-Invalid-Marker entfernt.
...
Die lokale Master-Zeile zeigt immer die vom aktuell laufenden API-Prozess importierte PBGui-Version. Ein stale Monitor-Agent-Cache kann weiterhin letzte bekannte Package- oder PB7/PB8-Details liefern, aber die lokale PBGui-Version nicht mehr durch einen veralteten Wert ersetzen. Update-Button-Farben bleiben Commit-basiert und benoetigen einen verifizierten gecachten Upstream-Head; stale Cache-Text allein behauptet niemals, dass ein Update verfuegbar ist.
...
PB8 verwendet `<install_dir>/pb8` und `<install_dir>/venv_pb8`, validiert PB8-CLI, Rust-Erweiterung, V8-Config-Schema und Source-Fingerprint und speichert danach `pb8dir` und `pb8venv` in `pbgui.ini`. Lokale und entfernte Master erhalten das vollstaendige PB8-Profil fuer Backtest und Optimize. Verwaltete VPS-Runner erhalten nur das minimale Live-Profil, verwenden pip ohne Download-Cache, entfernen nach der Validierung das temporaere Rust-Buildverzeichnis und aktivieren den gemeinsamen PBRun-Controller fuer PB8-Live-Supervision. Ein vorhandener verwalteter PB8-Live-Bot wird genau einmal nach erfolgreicher vollstaendiger Update-Validierung neu gestartet, damit er die neue PB8-Version laedt; ein fehlgeschlagenes Update loest diesen Restart nicht aus. Eine erste Remote-PB8-Installation verlangt mindestens 3 GiB freien Speicher. Eine bereits validierte PB8-Runtime bleibt auch unterhalb dieser Erstinstallationsreserve aktualisierbar. Das Playbook protokolliert vor und nach beiden Operationen freien Speicher sowie die Groessen von Checkout und Venv. Die RAM-Groesse ist keine Installationssperre.
...
Bulk-Updates in der Overview richten sich danach, was auf jeder ausgewaehlten VPS installiert ist. **Update Passivbot (PB7/PB8)** und **Update PBGui + Passivbot (PB7/PB8)** senden PB7-only-Hosts an PB7-Playbooks, PB8-only-Hosts an PB8-Playbooks und kombinierte Hosts an geordnete PB7+PB8-Playbooks. Gemischte Selektionen behalten damit einen gemeinsamen parallelen oder sequenziellen Rollout, ohne PB7-Tasks auf PB8-only-Hosts anzuwenden. Rollout-Aktion, Modus, Debug- und Reboot-Auswahl werden sofort gespeichert und nach einem Seiten-Reload wiederhergestellt.
...
In der Bot-Tabelle des Bereichs **Monitor** steht **HL Limit** direkt vor **CPU** für jeden zugeordneten Hyperliquid-Bot, ob PB7 oder PB8. Der Nutzungsprozentsatz steht oberhalb der tatsächlichen Used/Cap-Zähler. Bots mit derselben Wallet zeigen dasselbe Adresslimit; bei Bots ohne bekanntes Hyperliquid-Konto steht ein Strich. Die VPS mit laufendem Bot fragt `userRateLimit` alle fünf Minuten einmal je lokaler Wallet-Adresse ab; Master lesen den Cache des VPS-Agenten. **nRequestsUsed**, **nRequestsCap**, verbleibende Requests und Messzeit beziehen sich auf das kumulative adressbezogene Aktionslimit, nicht auf das REST-Gewichtslimit der IP. Bei einem Abfragefehler bleibt der letzte erfolgreiche Wert mit **Stale** gekennzeichnet. Die Browseranzeige liest nur den Cache und erzeugt keine zusätzlichen Hyperliquid-Abfragen. Ein Klick auf den HL-Limit-Wert eines Bots öffnet den 24-Stunden-Verlauf für **Used** und **Cap** im selben Diagrammfenster wie CPU und Memory. Erfolgreiche Messungen werden auf dem Master gespeichert und bleiben nach einem API-Neustart erhalten; Lücken zeigen ausgefallene Messungen. Der Verlauf beginnt mit der ersten Messung nach Installation dieser Funktion. Im Verlaufsfenster lässt sich außerdem eine genaue ganze Anzahl Request-Credits (1–100.000) eingeben. PBGui zeigt den Preis von 0,0005 USDC pro Credit an. **Buy credits** verlangt eine ausdrückliche Bestätigung und signiert danach einmalig die Aktion `reserveRequestWeight` mit den gespeicherten Hyperliquid-Zugangsdaten. Der Betrag wird vom Perps-USDC-Guthaben dieses Hauptkontos abgezogen; Vault-Konten werden nicht unterstützt. Bei unklarem Börsenergebnis versucht PBGui den Kauf nicht erneut. Nach erfolgreicher Aktion fragt PBGui das Limit sofort neu ab und aktualisiert das Diagramm automatisch, sofern die Abfrage gelingt.
...
| **Check Linux Updates** | Ueber VPSMonitor eine aktuelle read-only Paketsimulation ausfuehren und den gecachten Zaehler samt Details dieses Hosts sofort ersetzen |
...
Kombinierte PBGui-/Runtime-Updates verschieben den API-Neustart ueber eine transiente systemd-Unit, bis der vollstaendige Ansible-Runner beendet ist. Die Restart-Unit deaktiviert systemds eigene Environment-Expansion, damit Bash-PID-/Startzeit-Pruefungen unveraendert bleiben und keinen spaeteren PB7-/PB8-Installationsschritt desselben Playbooks beenden koennen.
...
- Automatische Paket-Probes laufen auf jeder VPS unabhaengig stuendlich; VPSMonitor liest diese Caches normalerweise ebenfalls nach seinem eigenen Stundenintervall ein. Hosts koennen deshalb kurzzeitig Zaehler aus unterschiedlichen Erzeugungszeitpunkten zeigen. Vor der Upgrade-Simulation aktualisiert der Collector einen unprivilegierten apt-Index, der privat im Monitor-Agent-Datenverzeichnis von PBGui liegt; der System-apt-Cache und installierte Pakete bleiben unveraendert. **Check Linux Updates** umgeht beide Stundenintervalle und liest den neu erzeugten Cache sofort ein.
- Ein Klick auf einen von null verschiedenen **Updates**-Wert in der Overview oder im Host-Header öffnet die gecachte apt-Paketliste einschließlich neu installierter Abhängigkeiten und geplanter Entfernungen. Die Gesamtzahl enthält sowohl die von der simulierten Transaktion ausgewählten Pakete als auch von apt als `not upgraded` gemeldete Pakete, etwa phased oder gehaltene Updates. Zurückgestellte Pakete werden separat angezeigt und bleiben unklassifiziert, weil apt dafür keine Transaktionsdetails ausgibt. Security-Updates werden zur zeitnahen Installation markiert, Entfernungen verlangen eine Prüfung der Abhängigkeiten und Dienste, Kernel-Updates empfehlen ein Wartungsfenster und gegebenenfalls einen Reboot, Routine-Updates können in der normalen Wartung installiert werden. Ältere Agent-Caches bleiben lesbar, zeigen aber nur die Anzahl, bis PBGui den Agent-Payload aktualisiert.
