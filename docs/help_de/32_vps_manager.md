...
7. Nach erfolgreichem Setup registriert PBGui den Host lokal als Cluster-Node-Kandidat. Klicke auf der VPS-Detailseite **Add to Cluster**, um das Onboarding abzuschliessen. PBGui aktiviert automatisch die gespeicherte SSH-Verbindung, repariert die eingeschraenkten Cluster-Keys, prueft auf eine kollidierende Identitaet, joint und synchronisiert den Node, materialisiert zugewiesene Configs/API-Keys und startet PBRun, sobald der Remote-State aktuell ist.
