...
Nach Bestätigung von **Enable Live** läuft die Aktivierung beim Wechsel zu einem anderen Sidebar-Account weiter. **Enabling Live…** markiert den laufenden Vorgang; nach Abschluss aktualisiert sich der Modus, Fehler bleiben am betroffenen Account sichtbar. Pro Account kann nur eine Aktivierung gleichzeitig laufen. Die Seite bis zum Abschluss der Vorgänge geöffnet lassen.

Die Account-Spalte richtet Namen, Modus-Badges und Währungen an festen Positionen aus. Lange Namen werden optisch gekürzt; der Tooltip zeigt den vollständigen Namen. Bei Snapshot-Problemen benennt der Status-Tooltip die betroffene Abfrage und den Grund. **Account mode** bedeutet, dass der Adapter den Kontomodus für Übertragungen nicht unterstützt, auch wenn Guthaben lesbar sind. Ein Fehler bei Historienabfragen oder Modusprüfungen bedeutet nicht, dass die angezeigten Guthaben nicht geladen werden konnten. Eine fehlgeschlagene Modusabfrage bleibt ein Lesefehler und belegt keinen inkompatiblen Kontomodus.

Die Hyperliquid-Übersicht fragt nur Guthaben und Kontomodus ab: drei Requests für einen normalen Account, mit gemeinsamen Leader-Abfragen für Vaults. Historien, Orders, Rolle und Spot-Metadaten entfallen hier. Die Übersicht begrenzt ihre Requests accountübergreifend auf höchstens 2,5 pro Sekunde. Bei Timeouts, Verbindungsfehlern, HTTP 429 und vorübergehenden Serverfehlern gibt es höchstens drei Versuche; dauerhafte Fehler werden nicht wiederholt. HTTP 429 pausiert alle Übersichtsabfragen und berücksichtigt einen numerischen Retry-After-Wert bis 60 Sekunden. Bei wiederholten Fehlern wächst das Aktualisierungsintervall vom eingestellten Intervall auf höchstens dessen Vierfaches, mit einem kleinen accountabhängigen Zeitversatz. Diese Grenzen gelten für die Übersicht, nicht für andere Dienste oder Bots. Quell- und Zielguthaben behalten getrennte Zeitstempel des letzten erfolgreichen Abrufs; ein Teilfehler verwirft nicht den anderen frisch gelesenen Wert. Live-Transferprüfungen bleiben unverändert.

Über **Balance refresh** im Header der Übersicht lassen sich **5, 15, 30 oder 60 Minuten** wählen (Standard: **15 Minuten**). Die Einstellung gilt für alle Übersichtsaccounts, wird sofort gespeichert und bleibt nach einem API-Neustart erhalten. Sie steuert die Guthabenabfragen im Hintergrund, nicht den Profit-Sweep-Zeitplan der einzelnen Accounts. Die Stale-Schwelle passt sich ebenfalls an; bereits laufende Abfragen dürfen noch fertig werden.

**Refresh now** fordert eine Aktualisierung der Kontostände im Hintergrund an, ohne auf das eingestellte Intervall zu warten. Die Abfragerate bleibt begrenzt; Zeilen aktualisieren sich nach Abschluss ihrer Abfrage. Anforderungen innerhalb einer Minute werden zusammengefasst; der Knopf wertet keine Sweeps aus und führt keine Transfers aus.

Die letzte Tabellenzeile zeigt **Summary** für die aktuell sichtbaren Accounts. USDT und USDC bleiben getrennt; simulierte Werte für Net PnL/Swept/Sweep Due tragen `sim`. Gemeinsame Ziel-Wallets zählen über eine anonyme Wallet-Kennung nur einmal; bis ein neuer Snapshot ein Hyperliquid-Ziel identifiziert, ist es von der Zielsumme ausgeschlossen. Ältere Cache-Einträge ohne Zielkennung erhalten einmalig eine sofort eingeplante Hintergrundabfrage unter Einhaltung der Abfragerate. Ein fehlgeschlagener Versuch wird ebenfalls gespeichert und löst keine Wiederholungsschleife aus; weitere Versuche folgen dem eingestellten Intervall und den Fehlerwartezeiten. `*` kennzeichnet eine Teilsumme mit fehlenden oder nicht identifizierten Werten. **Anonymize accounts** ersetzt Namen in der Übersicht und ihrer Sidebar einschließlich Namen-Tooltips durch stabile Aliase wie `Account 01, 02 …`. Beträge bleiben sichtbar, ein Zeilenklick öffnet weiterhin den richtigen Account. Die Checkbox ist eine visuelle Einstellung der aktuellen Seitensitzung; Accountdetails zeigen die echten Namen.

## Account-Übersicht

Die Seite öffnet **Overview**, auch über den obersten Button in der gemeinsamen Sidebar erreichbar. Jeder aktive Account mit **Live**, **Dry** oder **Paused Unknown** belegt eine Zeile: **Account / Mode / Asset**, **Status**, **Balance**, **Net PnL**, **Swept**, **Sweep Due**, **Target** und **Target Balance**. Ein Klick oder Enter/Leertaste auf der Zeile öffnet die Accountdetails. Deaktivierte Accounts bleiben in der Sidebar erreichbar.

Guthaben werden sofort aus einem privaten, dauerhaft gespeicherten Cache geladen. Ein Hintergrunddienst der API aktualisiert Accounts unabhängig voneinander mit höchstens drei parallelen Abfragen und dem eingestellten Aktualisierungsintervall je Account (Standard: 15 Minuten). Abfragen gemeinsamer Hyperliquid-Vault-Zielkonten werden bis zu einer Minute zusammengefasst. Solange die Übersicht sichtbar ist, fragt der Browser alle fünf Sekunden die lokal verfügbaren Werte ab. Diese Abfragen werten keine Policies aus und lösen keine Übertragungen aus. Beim Beenden der API werden die Worker gestoppt und abgewartet; die letzten Guthaben bleiben nach einem API-Neustart verfügbar.

**Rounding** und **Recovery** sind Wartezustände, keine Fehler. **Paused** und **Read error** erfordern Aufmerksamkeit. **Stale** kennzeichnet fehlende oder über das eingestellte Intervall zuzüglich Toleranz hinaus veraltete Guthaben-Snapshots; `—` bedeutet nicht verfügbar, niemals null. Nach einem Lesefehler bleibt das letzte bekannte Quellguthaben mit seinem ursprünglichen Zeitstempel erhalten. Der Status-Tooltip zeigt die letzte Entscheidung, den aufzuholenden Verlust, Reference Capital, High-Water Mark, maximal übertragbaren Betrag, letzten erfolgreichen Sweep, nächsten Check sowie getrennte Aktualisierungszeiten für Guthaben und Sweep-Zahlen.

Beträge erscheinen mit zwei Nachkommastellen, exakte Werte im Tooltip. Die Währung steht beim Account, da USDT und USDC vorkommen können. **Net PnL** bezieht sich auf die aktuelle Dry- oder Live-Baseline; **Swept** zeigt die bestätigte Summe des aktuellen Live-Zustands oder die ausdrücklich mit `sim` gekennzeichnete Dry-Simulation. Baseline-Resets können diese Abrechnungszeiträume verändern. **Sweep Due** ist der aufgelaufene Anspruch, keine bereits beauftragte Übertragung. Sweep-Zahlen folgen den vorhandenen geplanten Auswertungen; Guthabenabfragen berechnen sie nicht neu. **Target Balance** zeigt den aktuellen Zielbestand einschließlich anderer Ein- und Auszahlungen. Bei einem Vault ist Balance der Equity-Anteil des Leaders in diesem Vault, nicht dessen gesamte TVL.
...
- **Bitget Spot zeigt unavailable:** Ein erfolgreich gelesenes leeres Ziel-Wallet wird als `0` angezeigt; `Unavailable` bedeutet, dass Bitget den Read abgelehnt oder eine unbrauchbare Antwort geliefert hat. Wallet Transfer reicht zum Verschieben der Funds. Bitget Spot read muss aktiviert sein, wenn PBGui einen nicht leeren Spot-Saldo anzeigen und Transfer-History abfragen soll. Liefert Bitget eine erfolgreiche synchrone Transfer-ID, waehrend History-Reads verboten sind, bestaetigt PBGui anhand dieser Exchange-Antwort ohne erneute Submission.
...
- **Unknown Operation:** Nicht erneut senden und nicht zuruecktransferieren. Zeitpunkt und Betrag mit der Exchange-History vergleichen, Logs oeffnen und **Reconcile** fuer den bestehenden Live Intent oder die Testoperation verwenden. Die Bitget-UTA-Reconciliation liest das `OTHER`-Kapital-Ledger und akzeptiert genau einen Treffer mit passendem Asset, Betrag, Richtung und Zeitfenster, ohne erneut zu senden. Bitget vergibt unterschiedliche IDs fuer die synchrone Transfer-Bestaetigung und den Kapital-Ledger-Eintrag; PBGui behaelt deshalb beide, statt ihre Gleichheit zu verlangen.
