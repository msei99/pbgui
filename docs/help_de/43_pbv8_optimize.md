...
- Die offizielle **Convert to V8**-Migration steht fuer PBv7-Optimize-Configs bereit. Die vollstaendige Config wird an PB8 uebergeben; blockierend sind nur Befunde aus `optimize`, `backtest` und `bot`, die eine Optimize-Auswertung beeinflussen koennen. Reine Run-`live`-Befunde blockieren diesen Kontext nicht. PBGui-Metadaten und der redundante Legacy-Default `max_pending_starting_evals_per_cpu=1` werden vor der Migration entfernt. Echte Fehler zeigen eine begrenzte Liste von Feldern und Verhaltenswarnungen statt des vollstaendigen Migrationsreports.
...
Suite-Summaries behalten ihre konfigurierten Objective- und Szenarionamen und unterstuetzen `mean`, `min`, `max`, `std` und `median`. Der **Columns**-Picker steuert die sortierbaren Listenmetriken und merkt sich die PB8-Auswahl. Die Defaults enthalten kanonischen Gain, konfigurierte Objectives und kanonischen Drawdown; optionale kompakte Vergleiche umfassen ADG, Sharpe, Sortino, Omega, Loss/Profit Ratio und Equity/Balance Divergence, sofern PB8 sie gespeichert hat. Kanonische Werte bevorzugen die etablierten PB8-Aliase, beispielsweise `gain_usd` vor `gain_strategy_eq` und `drawdown_worst_strategy_eq` vor USD-/Fallback-Drawdown. Statistiken verwenden wenn vorhanden den angeforderten Metrik-Statistikwert und sonst das gespeicherte skalare Objective. Nicht zugehoerige Suite-Statistiken werden nicht an den Browser uebertragen. Ein kompakter Dateisignatur-Cache beschleunigt wiederholte grosse Pareto-Listen; geaenderte, geloeschte, fehlerhafte oder gerade neu geschriebene Kandidaten werden einzeln behandelt.
