...
PB8 waehlt den Beschleuniger automatisch; `optimize.backend: "gpu"` bleibt bestehen. Fuer Vast.ai-GPU-Jobs bleiben **Population size**, **Batch size** und **Max candidate bars** bei aktiviertem `auto_lean_parallelism` leer; PBGui waehlt die Werte nach Miete und Messung der GPU. PBGui sucht automatisch nach einer gespeicherten Messung derselben Arbeitslast und GPU-Variante einschliesslich des von Vast gemeldeten Powerlimits und zeigt eine Dispatch-Vorschau. **Apply measured suggestion** uebernimmt alle drei Werte nur auf Wunsch. Die Vorschau ist eine Schaetzung, kein Vorab-Benchmark und keine Garantie fuer genau einen Batch. Ohne passende Messung verwendet PBGui sein konservatives Hardwareprofil; nach dem Lauf Durchsatz, Leistungsaufnahme und VRAM pruefen. Andere Vast-GPU-Einstellungen wie Exact-Validierungen und Drift-Schutz behalten ihr bisheriges Verhalten. Passende Apple-M3-Workloads behalten ihr separates Tuning. Exakte Rust-Validierung bleibt massgeblich. Resume behaelt PB8s Runtime-/Checkpoint-Kompatibilitaetspruefungen bei; bei inkompatiblen Runtime-Versionen oder Beschleunigerwechseln einen neuen Lauf starten. Die GPU-Metrikauswahl beruecksichtigt Aliase, die der installierte PB8-Validator akzeptiert, darunter `long_short_profit_ratio` als Alias fuer `pnl_ratio_long_short`; Exact-only-Metriken bleiben ausgeschlossen.

Bei ausgewaehltem GPU-Backend zeigt der Editor PB8s Runtime-Defaults fuer nullable Population-, Batch- und Candidate-Bar-Groessen, M3 Lean Auto-Parallelism, Exact-Worker-, Drift- und Checkpoint-Controls. **Staged history** ist eine optionale Suchmethode, keine GPU-Groesseneinstellung: `0.25, 0.5, 1` wertet Kandidaten auf 25 %, dann 50 % und schliesslich 100 % der Historie aus. Die drei Werte erscheinen nur bei aktivierter Funktion fuer einen lokalen GPU-Job. Vast.ai unterstuetzt sie nicht; dort bleibt die Option verborgen, ausser wenn sie in einer aelteren Konfiguration abgeschaltet werden muss. Sie verwenden das normale responsive Acht-Spalten-Raster des Editors: 8×1 Felder auf breiten, 4×2 auf mittleren und 2×4 auf kleinen Bildschirmen. Bei lokalen GPU-Jobs behalten leere Sizing-Felder PB8s automatische Defaults und zeigen den effektiven Runtime-Wert als Platzhalter `auto (…)`; Vast-GPU-Jobs zeigen bei leeren Feldern `auto after rent`. Bei den drei optionalen Groessenfeldern fuehrt **−** von 1 zurueck zu Auto und **+** von Auto zu 1. **Reset GPU defaults** stellt die Defaults der installierten Runtime wieder her, ohne unbekannte zukuenftige GPU-Keys zu loeschen. Neue Scoring- und Limit-Auswahlen verwenden PB8s GPU-Proxy-Allowlist; vorhandene inkompatible Eintraege bleiben zur Reparatur sichtbar und PB8s nativer Preflight blockiert sie vor Queue oder Start.
...
Queue Backtest und Queue Validation senden die ausgewaehlten PB8-Jobs mit einem dauerhaften Backend-Request und lassen Diagramm, Filter und Auswahl bestehen. Der Server reiht die Jobs auch nach Seitenwechsel oder Browser-Refresh weiter ein; bei der Rueckkehr zu Optimize erscheint der Batch-Fortschritt automatisch. Open Queue zeigt die bereits hinzugefuegten Jobs. Autostart kann sie schon waehrend des Einreihens starten. Jeder Request erfasst die ausgewaehlten Konfigurationen, exakten Validierungszeitraeume und Overrides. Derselbe Batch setzt unbestaetigte Jobs mit stabilen Vorgangs-IDs fort; bei einem Fehler wird der betroffene Job angezeigt. Das Hinzufuegen sendet selbst keinen Startbefehl.
...
Ein vorbereiteter, inaktiver PB8-GPU-Queue-Job bietet **Tune GPU with this frozen input**. Das ist ein optionaler, getrennt bestätigter kostenpflichtiger Test mit genau den vorbereiteten Daten, kein Teil eines normalen Optimizer-Starts. Er erstellt eine isolierte Kopie; der ursprüngliche Queue-Job bleibt unverändert. Zuerst ein GPU-Angebot auswählen. Die Aktion benötigt den separat veröffentlichten Protokoll-4-Worker. Siehe [GPU-Leistungstest-Anleitung](48_vast_gpu.md#optimizer-settings).
