...
| **🔎 Apply Filters** | Apply `market_cap`, `vol/mcap`, Tags, `only_cpt`, and `notices_ignore` across all selected exchanges and replace both Long/Short approved and ignored lists with the combined result |
...
Filter values are configuration metadata until **Apply Filters** is clicked. Saving alone preserves the numbers but does not recalculate explicit approved/ignored lists.
...
The **Suite Mode** section now reuses the same shared component as Backtest instead of leaving suite-only configs stranded in a legacy page. That means `backtest.suite_enabled`, `backtest.scenarios`, and `backtest.aggregate` can now be edited and saved directly in FastAPI Optimize, including the built-in templates, scenario override rows, per-scenario `coin_sources`, and aggregate metric rules. Active scenario fields are retained automatically when adding, moving, or removing scenarios or disabling Suite Mode; **Done** is not required before those actions.
...
| **crossover_probability + mutation_probability** | The DEAP editor keeps the combined probability at or below `1.0`, matching the legacy optimizer behavior |
...
| **Settings** | Open the queue settings dialog for `Autostart`, the CPU value autostart should enforce, and the `Use PBGui Market Data` launch override |
...
The compact **pareto list** action now uses its own folder-style icon, while **🎯 Pareto Explorer** is restored as a separate dedicated action again, matching the old results-table layout instead of overloading the list button with the explorer icon. When you click **🎯 Pareto Explorer** from Results, PBGui stays in the current tab and forwards the selected result path automatically. When you return to FastAPI Optimize afterwards, the page refreshes its configs, queue, and results automatically so the Results view does not stay on a cached empty snapshot.
...
For suite results, the **Summary** badges now follow the toolbar selection instead of always showing one fixed aggregate view. That means you can inspect either the aggregated suite statistic or one specific scenario directly in the FastAPI page without switching to a legacy Optimize page.
