...
- Backtest handoffs normalize legacy null-like directory values such as `None` to PB8's canonical `logging.dir = logs` and `monitor.root_dir = monitor` defaults, so newly created live instances do not write into accidental `None` directories.
...
PBv8 Run uses the same **Backups** workflow as PBv7. Before an existing instance is overwritten or deleted, PBGui stores the complete previous bundle under `data/backup/v8`: `config.json`, every referenced sparse override file, and any available supervisor or PB8 runtime log. The retention setting controls how many versions are kept per instance.

**Load in editor** creates a short-lived draft for review and normal saving. **Rollback** is available for an existing instance and requires the bot to be verifiably stopped locally and on every relevant remote host. It backs up the current bundle, restores the selected `config.json` and all referenced sparse overrides, publishes a new monotonic Cluster version, and keeps the restored instance disabled. Missing, stale, or ambiguous runtime observations reject the rollback with HTTP 409 instead of guessing. Deleting a backup affects only that immutable backup bundle.
