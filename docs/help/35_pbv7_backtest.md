...
The Configs table also offers **V8** for the saved V7 backtest config. Both conversions leave the V7 source unchanged and pass the complete config to PB8. When converting a result, PBGui derives the effective maker and taker rates from its linear-market `fills.csv` data before migration, preventing a normalized result default from replacing the exchange fees actually used by V7. Before migration, PBGui omits retired live-only execution fields, canonicalizes `backtest.aggregate` to `backtest.reducer`, and removes a duplicate singular exchange when it already matches `backtest.exchanges`. After PB8's official migration, PBGui removes strategy-incompatible optimizer overrides, canonicalizes fixed runtime override paths, freezes already disabled bot sides, and restores V7's implicit positive-threshold WEL/TWEL enforcer behavior. If PB8's `bounded` WEL excess mode would reduce V7 position sizing, the affected Long/Short JSON is marked and Save remains blocked until the user explicitly chooses `legacy_raw` for V7 parity or `bounded` for the PB8 safety cap. A positive `minimum_coin_age_days` is also marked because PB7 uses global coin age while PB8 uses exchange-specific market age; the user must choose `0` for PB7 data coverage or retain the original value for PB8's age gate. Deterministic changes are recorded as `ok_with_adjustments`; conflicts remain manual-review blockers. Every successful conversion opens an unsaved PB8 Backtest editor draft and never creates or replaces a V8 config by itself. The migration report remains attached to the draft and is written beside the config only after the user explicitly clicks **Save** or **Save & Queue**. Backtest review excludes Live and Optimize findings but retains strategy Bot findings because they affect the simulated result. If genuine context-relevant fields remain, the draft marks available structured fields red; invalid output remains blocked.

### Result charts

Clicking a row opens a full-featured chart panel with:
- **Equity curve** (log scale toggle)
- **Price (PBGui MarketData)** selector for overlaying one result exchange/coin close-price series as a muted dotted trace on a contrasting second Y-axis; coverage is evaluated against the visible equity-chart range, and partial or unavailable data is shown beside the selector while the equity chart remains visible
