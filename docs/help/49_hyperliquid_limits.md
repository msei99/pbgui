...
Open an active account row to see its 24-hour Used and Cap history. History is collected on the Master only while a bot is running for the account. An account without a running bot is listed as **Not sampled**. To check such an account once, open its saved entry under **System → API-Keys**; PBGui reads the limit once from Hyperliquid and displays it next to **Test Connection**. That one-time read does not create ongoing history. Opening the API key again performs another single read.
