...
For a new Vast GPU optimizer job, leave `optimize.gpu.population_size`, `batch_size` and `max_dispatch_candidate_bars` blank with `auto_lean_parallelism` enabled to let PBGui select a coordinated profile after the rented worker reports its actual GPU. These fields are optional for Save and Save & Queue. To override automatic sizing, enter explicit values. **Apply measured suggestion** fills all three fields only for a matching selected offer and never changes a job silently; the offer match remains preliminary until runtime verification. The editor previews dispatches and the candidate-bar budget only when explicit values are present. A real optimizer run does not conduct a tuning pretest.

Open the PB8 configuration and choose **Run on → Vast.ai GPU**. Under **GPU optimization → Population & dispatch**, leave the three sizing fields blank for automatic post-rental selection, or set them manually before **Save & Queue**. On these three fields, **−** from 1 returns to Auto and **+** from Auto starts at 1. Explicit configuration values remain fixed for that queued job.

Selecting an offer under **GPU & Offers** shows every prepared waiting job with the Auto population, batch size and work limit calculated for that job and selected card. The work limit is displayed and edited in billions of candidate-bars; PBGui saves the exact integer. For each Auto-sized job, choose Auto, a shipped measurement for the exact workload and GPU/VRAM variant, or custom values. A measurement at a different power limit is clearly marked as preliminary. The selected offer is not yet a runtime-verified GPU; its advertised power and the actual rented card can differ. Explicit sizing already saved in a job config stays authoritative. Manual Rent saves the choices per job; before the first job starts, they can still be edited under **Rental & Automation** or via **GPU profile** on the Queue card. New jobs added later use Auto sizing. No optimizer pretest is run.
...
The GPU column also shows Vast's advertised power limit before rental, or **not reported** when Vast omits it. This offer value is not a live measurement; the configured limit is verified from the rented GPU during operation. Completed performance-test profiles include the measured configured power limit. On a real rental PBGui prefers an exact profile, then the nearest power-limit profile for the same runtime GPU and exact VRAM variant; modified-VRAM cards are not mixed.

Selecting an offer also shows its GPU performance-profile coverage beside **Rent**. PBGui distinguishes a **Local test**, a versioned **PBGui reference** shipped with the application and the existing conservative default. The marketplace match is preliminary. After rental, the worker verifies the runtime GPU name, exact VRAM in MiB, compute capability and device identity; same-name cards with non-standard VRAM remain separate variants.
Completed test measurements and **Use completed test profile** are shown here only when the selected offer has the same advertised GPU name and VRAM as the tested card. Select the matching GPU offer to review its result; the Queue job retains all historical results independently of the offer selection.

**Run performance test** uses PBGui's versioned EMA-anchor comparison workload: BTC, ETH and SOL on Binance in 2024, with fixed seed and objective. Normal optimizer jobs are never tested automatically. The separate, explicitly confirmed queue-item tuning action uses its frozen prepared input without changing or starting the original job. PBGui prepares and freezes local market data only after confirmation, then rechecks the selected offer before renting. The new protocol starts at 4,096 and 8,192 candidates, then tests 12,288, 16,384 and further 4,096 steps while scaling remains worthwhile. A gain of at least 10% continues; a smaller positive gain keeps the new population, while a regression keeps the preceding one. The 15% projected VRAM reserve, rental deadline and 131,072 safety ceiling still apply. The isolated rental is removed after the test.
Each result shows candidate rate, the completed-generation measurement window, GPU proxy time and effective dispatch size/count when PB8 reports them. Protocol-3 accepts a population measurement only when PB8 confirms its full requested batch fits one dispatch. A capped case cannot become a reusable recommendation. Case wall time is not necessarily the rate-measurement window. Older trailing-martingale protocol-2 results remain visible and usable but are labelled as legacy evidence; their sparse optimizer-log rate can reflect only one logged generation. EMA and trailing-martingale results are not directly comparable. Accepting a completed result with **Use completed test profile** creates a local override for the exact runtime-verified GPU variant and measured power limit without changing bundled references. Repeats preserve earlier evidence. The published calibration worker is pinned by immutable digest for new tests.

**Reference workload** offers Small (3 coins/Binance/one 2024 scenario), Medium (10 coins/two exchanges/two scenarios) and Large (41 coins/two exchanges/four 2024 scenarios). **Adjust test search** changes the starting population, step, ceiling, minimum gain and per-case timeout before confirmation. The unchanged Small preset keeps the published protocol-3 test. The published and pinned protocol-4 worker also enables configurable tests; PBGui never rents on a version mismatch. Waiting tests currently use only the fixed Small reference. Protocol 4 measures at least one complete GPU generation per population and rejects any scenario whose requested population was split into multiple dispatches. Its displayed recommendation is the best validated search step under the selected stopping rule, not a universal optimum. Population and batch are equal; the stored work cap is the largest measured candidate-bars of the selected step and will split larger real workloads. The exact workload fingerprint prevents applying the measured rate to unrelated configurations.

The reference workload and search controls stay hidden during ordinary rental. Select **Performance test…** to open them, then confirm **Run performance test** or arm a waiting test.

For an already prepared, inactive PB8 GPU Queue item, **Tune GPU with this frozen input** creates a separate test copy only after a selected offer and explicit rental confirmation. It reuses the immutable prepared files, does not prepare data again, and never starts or changes the original optimizer job. Its measured result is shown on the test Queue item. Accepting it saves a workload-specific local profile; it does not silently rewrite the original queued snapshot. Ordinary queued jobs never run a tuning pretest.

Input preparation begins only after that confirmation and appears as a preparing calibration entry in Queue with copy and compression progress. No paid rental starts until preparation has completed and PBGui has rechecked the selected offer. A disabled action states its reason prominently beside the buttons. An enabled automatic GPU scheduler is paused as part of the confirmed test instead of blocking an otherwise idle rental; an active rental, another calibration or unavailable rental supervision still blocks the action.

The GPU offer search also accepts minimum advertised power limit (W) and host reliability (%). **Test when matching GPU is available** uses the saved filters, selected rental hours and budget after a separate explicit approval. It may wait indefinitely until **Cancel waiting test**; waiting itself does not rent or charge. When an offer appears, PBGui rechecks the exact offer's identity, price and all frozen limits immediately before a rental attempt. If Vast rejects a stale offer and independently confirms that no instance was created, the search resumes; an uncertain attempt blocks further rental requests until supervision resolves it. The first confirmed rental ends the watch. GPU & Offers shows a compact status card with the frozen search limits and the current phase, including when a matching GPU was found and its test is starting. Saved rental settings and GPU requirements are collapsed below the actions but remain available on demand. The actual GPU power limit is measured only after rental; if it is below the approved minimum, the test stops and the rental is cleaned up instead of silently testing another card. An automatic GPU scheduler and an active rental cannot run alongside this watch.

Each calibration population uses its own working directory, with a copy of PBGui's staged market and first-candle caches; missing or expired metadata stops the test instead of triggering a live exchange lookup. Immediate and waiting tests require the pinned protocol-3 calibration worker, now published. PBGui checks its immutable digest before provider access rather than treating an ordinary optimizer run as calibration evidence.
...
**Open log** appears when a downloaded log exists or the cloud job is provisioning/running. Waiting cloud jobs have a trash action instead of Stop. Deleting an inactive queue entry removes its retry history, local job snapshots, logs and performance measurements; already imported optimizer results remain available in Results. Old inactive entries can also be deleted while another job is **preparing**. Entries disappear automatically before large snapshot folders finish deleting, and that file cleanup does not hold the queue lock or interrupt queue updates. If PBGui restarts during cleanup, it finishes marked deletion leftovers in the background after startup. Active jobs must finish stopping/collection before removal. Older entries already hidden by the previous deletion bug are also purged automatically at API startup. A hidden job linked to a visible or active retry is retained until the entire retry lineage can be removed safely.
