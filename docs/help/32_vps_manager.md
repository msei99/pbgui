...
| **Update PBGui and PB7 / Update PBGui and PB8** | Update PBGui together with the runtime present on a single-runtime host |
...
7. After setup succeeds, PBGui registers the host locally as a Cluster node candidate. Click **Add to Cluster** on the VPS detail page to complete onboarding. PBGui automatically enables the stored SSH connection, repairs restricted Cluster keys, verifies that no conflicting identity exists, joins and synchronizes the node, materializes assigned configs/API keys and starts PBRun when the remote state is current.
