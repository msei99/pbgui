...
**Scan all_results** is disabled while `all_results.bin` is unavailable. For Vast.ai runs, the full history becomes available after optimization finishes and the final results are downloaded. Hover over the disabled button for this explanation; the Pareto front remains usable during the run.
