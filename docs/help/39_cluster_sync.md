...
| **VPS runner** | Server that can run PB7/PB8 bots and store a local copy of cluster state. |
...
- PB8 config bundles and a separate PB8 desired-state lifecycle.
...
## PB7/PB8 bots and desired state

For each PB7 or PB8 instance, its runtime-specific desired-state map stores:
...
Retention** to view the cluster-wide policy and live automatic cleanup status.

The values in **Retention** and **History Days** are drafts until you click
**Save Retention Policy**. The allowed history window is 1 to 3650 days; the
default is seven days. The confirmation keeps the policy generation that was
shown with those draft values. If another operator changes the policy while
the dialog is open, the save is rejected and the current policy must be
reviewed again.

The available settings are:

- **Disabled**: builds and verifies checkpoints but never deletes history or blobs.
- **Enabled (automatic)**: continuously retains the configured operation-history window and removes unreachable blobs whenever all safety checks pass. Existing signed `oplog` and `oplog_and_blobs` policies are both treated as this automatic mode.

**Automatic cluster cleanup** is the primary operational display. It refreshes
from PBCluster's persisted status every five seconds without opening SSH
connections or advancing cleanup. It shows the current phase, committed
checkpoint and replica acknowledgements, retained and deleted operation/blob
counts, blockers, and the latest evaluation time. Maintenance is evaluated
after policy changes and at least hourly. **Run Node Diagnostics** under
**Advanced node diagnostics** is optional and only calculates the detailed
per-node projection.

The report columns mean:

- **Projection**: `dry_run` means the optional node diagnostic only calculated candidates and did not delete anything.
...
- **Required Blob Set**: count and digest of the exact config, secret, and sealed hashes required by the projected checkpoint and retained state. Equal digests prove required-blob convergence even when local garbage differs.
- **Local Garbage Blobs** and **Local Garbage Size**: projected locally stored unreachable blobs and their combined size before cleanup, or the values from the latest matching automatic GC evaluation after cleanup starts.
- **Blob GC Projection**: `projected` for the read-only simulation, or whether automatic blob GC is blocked by a state-based safety gate, ready, or complete. The projection also reports already deleted blob counts and bytes after completion.
...
**Run Node Diagnostics** projects blob candidates from the
...
status instead. **Run Node Diagnostics** never changes or deletes data.
...
After saving an enabled policy, PBCluster replicates it, commits a matching
checkpoint with acknowledgements from every active state replica, and applies
retention automatically. If a replica is unavailable or any state check fails,
the status changes to **Retention paused**, nothing is deleted, and maintenance
resumes automatically when the cluster is healthy. There is no time-based
activation or stability gate. Old unreachable blobs retain a one-hour technical
minimum age so an upload cannot be deleted before its referencing operation is
stored.

Changing the policy writes a signed cluster operation. Automatic retention does
not bypass safety checks: every active state replica must independently confirm
the same checkpoint, credential protocol v2 migration must be sealed, and the
checkpoint reducer must match full replay. A conflict automatically pauses
deletion.
...
The page is split into Overview, Setup, Nodes, Credentials, Bot State, Tombstones, Retention and Oplog sections. Bot State and Tombstones show PB7 and PB8 in separate tables, with independent sorting for each runtime. Node rows show whether `pb8_instances_v1` is advertised. It refreshes local status, nodes, desired state and recent oplog entries in the background, and updates changed cards and node-table fields in place instead of reloading the whole screen.

The Oplog lists the reverse of the cluster's replay order: newer timestamps first, with ties ordered by node (actor), then its numeric sequence. Sequence counters from different nodes are independent, not a shared global clock. Refresh reads the validated operation list under the history lock without an additional full state rebuild. Retained cluster data with a missing or damaged identity is reported as an error, not as empty history. Credential and policy entries show their non-secret identifier in the Target column. If only the Oplog request fails, Status, Nodes and Desired State continue to refresh while the history panel reports that it is unavailable.
...
- disabled-node removal for stale nodes that no longer own PB7 or PB8 configs
...
After operations and config blobs are synchronized, the Preview window shows separate **V7 Config Materialization Preview** and **PB8 Config Reconciliation Preview** cards. **Materialize V7 Configs** writes assigned, non-conflicted V7 JSON configs into remote `data/run_v7`. **Reconcile PB8 Configs** exactly reconciles the assigned `data/run_v8` JSON bundle, including backed-up obsolete-file removal and tombstoned-directory deletion. Preview actions are reset whenever the window closes or begins loading another node, so an action from an earlier node cannot remain available. Both manual retries refuse to run while remote state differs or required blobs are missing or invalid.

PB8 live configs use separate `UPSERT_PB8_CONFIG` and `DELETE_PB8_INSTANCE` operations, `pb8_instances` / `pb8_tombstones` desired-state maps, and exact bundle manifests for `data/run_v8`. A peer must advertise `pb8_instances_v1`; when active PB8 desired state exists, older peers are blocked instead of receiving an operation stream they cannot interpret. The PB8 table offers Start, Stop, stopped-only Move to capable nodes, and guarded Delete. Every action is bound to the displayed cluster generation; Delete additionally verifies stopped runtime observations and backs up the current bundle. Materialization requires the local PB8 checkout plus a current owner-only PB8 exchange-key projection. It atomically reconciles the complete JSON bundle, backs up removed files under `data/backup/v8`, and never starts or stops bots. PBRun starts after a manual materialization only when V7, PB8, and API-key previews are all current.
