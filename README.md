...
## v1.77 (unreleased)
- Changed: `System -> Services -> Workers` no longer shows a separate `Open Monitor` action for monitor-backed workers because the monitor now opens inline automatically in the detail pane.
- Fixed: embedded worker monitors in `System -> Services -> Workers` now stay mounted during worker status refreshes, preventing flicker and unintended Job Monitor log closes.
- Changed: monitor-backed workers in `System -> Services -> Workers` now render their linked monitor directly inside the worker detail pane instead of opening a separate page.
- Changed: Worker `Stop` and `Restart` actions in `System -> Services -> Workers` now ask for confirmation before dispatching the command.
- Changed: `System -> Services` now includes a dedicated `Workers` admin area with grouped queue, sync, and internal workers, including runtime stats, start/stop controls, and worker log or monitor access where available.
- Changed: PB7 Backtest `HLCVS Cache Cleanup` now also removes old `pb7/caches/ohlcvs/materialized` run datasets alongside `pb7/caches/hlcvs_data`, and the Settings modal now shows both cleanup targets.
- Changed: Hyperliquid Tiingo settings no longer present PBGui's internal request counters as authoritative Tiingo account usage; the widget is now explicitly labeled as a local tracker and shows a hard warning when Tiingo reports the hourly allocation as exceeded.
- Changed: Hyperliquid Tiingo usage cards now also surface an active `server_429` backoff window, so the UI shows when Tiingo is throttling despite the locally tracked request counters still being below their configured limits.
- Fixed: Hyperliquid TradFi `Auto-Map` now uses the effective active XYZ status instead of blindly skipping stale raw `delisted` entries from `tradfi_symbol_map.json`, and its equity name check now also accepts descriptive XYZ texts like `LLY tracks ... Eli Lilly and Company`.
- Fixed: FastAPI Hyperliquid TradFi status resolution no longer trusts stale `delisted` entries from `tradfi_symbol_map.json` when the XYZ instrument is still active in `mapping.json`; active symbols like `SP500` now resolve to an effective non-delisted status such as `pending` instead.
- Changed: FastAPI Hyperliquid `Already Have` now marks XYZ rows with the real `mapping` status and extends the type filter with `xyz only`, `xyz mapped`, and `xyz not mapped`, so entries like `no_provider` are no longer misreported as mapped.
- Changed: FastAPI `Already Have` -> `PB7 cache` now adds a small toolbar timeframe filter next to `Select All`/`Deselect`, so duplicated short-name rows such as `ADA` can be narrowed to `1m` or `1h` before selecting them.
- Changed: FastAPI `Already Have` now renders inventory coin labels as short names in the table, sidebar actions, and heatmap/OHLCV captions while keeping the full symbol internally for queue/delete requests.
- Changed: The FastAPI `Already Have` inventory sidebar is now button-only; queue/delete confirmations and errors no longer render as persistent sidebar callouts and instead use toast/notification or the existing dialogs.
- Changed: FastAPI Hyperliquid `Already Have` -> `l2Book` now keeps successful queue confirmations out of the sidebar callout and shows them only as toast/notification entries.
- Fixed: The embedded Hyperliquid l2Book Job Monitor now cancels running jobs through the correct `/api/jobs/cancel` endpoint instead of silently posting to a non-existent route.
- Fixed: FastAPI Hyperliquid `Already Have` -> `l2Book` now skips the heuristic AWS archive preflight warning for inventory-selected coins and queues those selections directly for worker-side verification.
- Fixed: Hyperliquid l2Book queue preflight no longer hard-fails when explicitly selected coins cannot be confirmed from the sampled AWS archive probe. Those selections are now queued with a warning so the worker can verify historical availability during download.
- Fixed: FastAPI Hyperliquid `Already Have` -> `l2Book` now normalizes selected inventory symbols like `ADA_USDC:USDC` before queueing `/queue-l2book-download-bulk`, so selected rows no longer fail with `No matching enabled coins selected`.
- Changed: In FastAPI `Already Have`, the Hyperliquid `l2Book` view now exposes a queue action for l2Book downloads instead of the unrelated `Build best 1m` action.
- Changed: FastAPI inventory heatmaps now keep mouse-wheel zoom disabled, while the Plotly modebar is shown only on hover instead of sitting permanently over the chart. Page scrolling over the overview and minute heatmaps now stays passive without losing access to the plot tools.
- Fixed: FastAPI `Already Have` OHLCV now normalizes Plotly wheel/relayout timestamps even when Plotly emits naive values with more than 3 fractional digits. Deep `1m` wheel zoom-outs no longer collapse into an empty mini-window or drift left by a timezone offset.
- Fixed: FastAPI `Already Have` OHLCV now treats Plotly relayout ranges without an explicit timezone as UTC before clamping or rerendering. Deep `1m` zoom-outs no longer jump back by the browser timezone offset or drift into the wrong time window.
- Fixed: FastAPI `Already Have` OHLCV now also checks whether the current visible same-timeframe window actually contains loaded candles, not only whether coarse coverage heuristics look sufficient. If a `1m` / `5m` / `15m` view is effectively empty, the same timeframe is reloaded and redrawn.
- Fixed: FastAPI `Already Have` OHLCV now fully rerenders the Plotly traces after same-timeframe lazy reloads. Newly fetched `1m` / `5m` / `15m` candles no longer stay invisible just because only the layout was updated.
- Fixed: FastAPI `Already Have` OHLCV now tracks loaded client-side time windows per fine timeframe instead of treating one layer as one continuous `min..max` block. This lets zoom-out checks detect uncovered gaps between separately loaded windows and fetch missing candles before the chart shows large empty regions.
- Fixed: FastAPI `Already Have` OHLCV now also reloads the same fine timeframe (`15m` / `5m` / `1m`) when zooming out inside that same timeframe but the currently cached client window no longer covers most of the visible range. This prevents large empty gaps that could appear even while the chart badge still said `1m`.
- Fixed: FastAPI `Already Have` OHLCV now clamps zoom and pan differently. Box/wheel zooms clip to the actually selected data overlap, while pans still keep their span at the edge. This avoids zooming back out to a wider candle window than the selected area.
- Fixed: FastAPI `Already Have` OHLCV lazy loads no longer replace the currently cached fine layer window in the browser. New `1m` / `5m` / `15m` windows are merged into the existing client-side layer, so candles do not disappear again when panning across recently visited ranges.
- Fixed: The FastAPI `Already Have` OHLCV pan clamp now preserves the current visible span when you hit the data edges instead of snapping back to the full layer range, and the clamp debounce is shorter. That avoids blank windows while dragging and removes most of the rubber-band feel at the boundaries.
- Improved: The FastAPI `Already Have` OHLCV chart now aborts stale lazy-load requests when the visible range changes again, avoids unnecessary relayout work while panning inside the same timeframe, and only auto-fits Y aggressively on real zoom changes. That makes interaction steadier and more responsive.
- Improved: The FastAPI `Already Have` OHLCV lazy loader now uses much smaller timeframe-specific fetch windows and returns only the actually requested fine layer. This cuts zoom payloads from multi-megabyte responses down to small targeted loads, so the chart feels noticeably faster.
- Fixed: The FastAPI `Already Have` OHLCV chart now preserves the user-selected Plotly drag mode across rerenders and clamps pans/zooms back to the real candle range. This avoids unexpected jumps back to `pan`, reduces empty-chart states, and makes the interaction feel more stable.
- Changed: The FastAPI `Already Have` OHLCV chart now opens in Plotly pan mode and no longer locks the Y axes, so the zoomed chart can be dragged vertically instead of staying fixed in place.
- Fixed: The FastAPI `Already Have` OHLCV iframe no longer gets served with literal `\"` escapes inside its HTML/JS template. The chart script now executes again instead of getting stuck on `Loading chart...`.
- Fixed: The FastAPI `Already Have` OHLCV chart now mirrors the Streamlit lazy-zoom path instead of embedding the full fine-resolution candle pyramid up front. Long histories open reliably again, and zooming now fetches finer candles on demand (`1d` -> `1h` -> `15m` -> `5m` -> `1m`).
- Fixed: Hyperliquid `Already Have` → `l2Book` no longer shows duplicate rows when the `Show no l2Book yet` toggle is enabled. Missing-row injection now uses the same normalized coin format as the inventory itself.
- Changed: Hyperliquid `Already Have` → `l2Book` can now optionally include enabled non-XYZ coins with no local/archive l2Book files yet. A new toolbar toggle next to `Select All` / `Deselect` controls this, and stays off by default.
- Fixed: Hyperliquid `Download l2Books` now excludes `XYZ-*` / TradFi symbols from the selectable coin list and from the `All` queue path, because l2Book archive downloads only apply to native Hyperliquid coins.
- Changed: Hyperliquid `Download l2Books` now uses the same FastAPI coin-grid selector as `Best 1m`, with filter, direct row clicks, bulk `Select visible` / `Clear all`, and mouse-drag range selection instead of the old compact dropdown.
- Fixed: The FastAPI `Best 1m` coin grids for Binance, Bybit, and Hyperliquid now interpolate fast mouse-drag movement across intermediate visible rows, so quick drag painting no longer drops coins between sparse cursor events.
- Fixed: The FastAPI Market Data sidebar entries `Best 1m` and `Download l2Books` now use the same button styling and active-state highlighting as the other sidebar menu entries, instead of keeping their old underline and separate blue shortcut palette.
- Changed: The FastAPI `Best 1m` coin pickers for Binance, Bybit, and Hyperliquid no longer rely on checkbox rows. The visible coin grids are now directly clickable and support mouse-drag painting across rows, so larger coin ranges can be selected or cleared much faster.
- Fixed: The FastAPI Market Data TradFi `Search ticker` price lookup now resolves Tiingo search hits with exchange suffixes such as `BNO:BAT` against the underlying Tiingo quote ticker like `BNO`, so valid search results no longer lose their price just because the search API and quote API use different ticker formats.
- Changed: The FastAPI Market Data TradFi `Search ticker` result list now also shows the current Hyperliquid price for the selected XYZ symbol below each Tiingo hit, so Tiingo and Hyperliquid prices can be compared before applying a mapping.
- Fixed: The FastAPI Market Data TradFi `Search ticker` result list no longer renders missing Tiingo quotes as `0.0000`; search hits without a quote now show the price as unavailable instead of implying a real zero price.
- Changed: The FastAPI Market Data TradFi `Search ticker` window now also shows the current Tiingo price for each search hit when Tiingo can provide a quote, making it easier to judge whether the selected ticker matches the XYZ symbol before applying it.
- Fixed: The FastAPI Market Data `Search ticker` flow for TradFi/Tiingo now stays inside the floating utility window, shows the editable query and visible Tiingo results there, and exposes inline `Apply` buttons like the original Streamlit dialog.
- Changed: FastAPI pages with destructive or blocking actions now use PBGui-styled in-page dialogs for Market Data status, Logging purge, Dashboard actions, API Keys actions, Services bucket deletes, and Optimize config/result deletes instead of browser-native confirm, alert, and prompt popups.
- Changed: The FastAPI `Best 1m` view is flatter now. Its coin/build area no longer sits inside an extra card frame, and the embedded Job Monitor runs in a lean in-page embed mode instead of rendering with a nested standalone page header.
- Changed: The shared FastAPI nav `Restart` action and the embedded `Best 1m` Job Monitor now use PBGui-styled dialogs instead of browser-native confirm and alert windows.
- Fixed: The embedded FastAPI `Best 1m` Job Monitor now auto-resizes to its content height, so the extra inner scrollbar is gone and scrolling stays on the outer Market Data page.
- Changed: The FastAPI `Best 1m` panel for Binance and Bybit now starts directly with the build form. The redundant intro text block and top `Refresh` button were removed.
- Changed: FastAPI Market Data delete actions now use a centered PBGui-styled confirmation window instead of the browser-native confirm popup, so selected coins, date-based deletes, and dataset clears match the page styling.
- Changed: The FastAPI `Already Have` `Delete by Date` dialog now uses a visible Backtest-style calendar button and shows the selected coin names in a scrollable scope list, making multi-coin deletes easier to verify before confirming.
- Changed: The FastAPI `Already Have` sidebar is button-only again. The old inline `Delete older than` date field was replaced by a `Delete by Date` button that opens a small dialog with the date picker and preview.
- Fixed: The FastAPI `Already Have` sidebar `Build best 1m` action now keeps a stable snapshot of the currently selected coins and marks the request as explicit selection, so the queue endpoint cannot silently fall back to all coins when the selection state drops out.
- Fixed: The FastAPI `Already Have` sidebar `Build best 1m` action now normalizes selected inventory/cache coin names before queueing, so directory-style values like `AIGENSYN_USDT:USDT` are turned into real build targets instead of producing empty Bybit/Binance downloads.
- Fixed: The FastAPI `Already Have` sidebar `Build best 1m` action now queues the explicitly selected inventory coins directly for Binance and Bybit, even when those coins are not currently enabled in Settings.
- Changed: In FastAPI `Already Have`, selecting one or more coins now exposes a sidebar `Build best 1m` action that queues those exact coins directly as a Best 1m download/build job for the current exchange.
- Fixed: FastAPI `Best 1m` now keeps the Job Monitor below the full build form, and explicit coin selections are honored again when queuing Binance/Bybit Best 1m jobs instead of silently falling back to all enabled coins.
- Fixed: The FastAPI `Best 1m` panel for Binance and Bybit shows the filtered Job Monitor again directly below the build form, so queued/running/done/failed Best 1m jobs are visible without switching to the separate Activity Log panel.
- Changed: The FastAPI Market Data `Already Have` table is now sortable by clicking the column headers. Sorting is client-side per dataset view and keeps the current row selection intact.
- Changed: The FastAPI Market Data `Already Have` coin table now follows the same mouse-selection pattern as FastAPI Backtest/Optimize tables: a click toggles one row, dragging across rows adds or removes whole ranges, and `Select All` applies to the currently visible table rows.
- Changed: The FastAPI Market Data `Already Have` delete tools moved from a separate content card into the sidebar (Delete selected, Delete older than + date picker, Clear dataset). The coin table now supports multi-select with toggle click, plus Select All / Deselect buttons above the table. Heatmap is hidden when more than one coin is selected.
- Fixed: FastAPI Market Data `Already Have` backend no longer crashes with `NameError: name '_get_missing_lag_minutes' is not defined`. The missing function is now imported from the heatmap module.
- Changed: The FastAPI Market Data `Already Have` area now mirrors the Streamlit inventory workflow inside FastAPI. It exposes dataset-specific sidebar buttons (`1m`, `PB7 cache`, plus `1m_api` and `l2Book` on Hyperliquid), keeps PB7 cache read-only, and adds the same inventory table, delete tools, coverage heatmaps, minute view, and optional OHLCV detail chart without falling back to Streamlit tabs.
- Fixed: The focused Hyperliquid FastAPI panel no longer shows an extra inner scrollbar from the embedded document defaults. Its embedded page now resets the root margins so scrolling stays on the outer Market Data page.
- Fixed: Hyperliquid `Best 1m` in FastAPI no longer jumps back upward when you reach the lower part of the panel. The embedded iframe now measures the focused content root directly instead of collapsing itself during live resize updates.
- Fixed: Switching the FastAPI Hyperliquid panel from `Best 1m` to `Download l2Books` now shrinks the embedded panel height correctly, so stale empty space and the extra page scrollbar from the taller previous view are no longer left behind.
- Changed: Hyperliquid `Best 1m` inside FastAPI Market Data now also uses the shared editor-style calendar inputs and the same multi-column coin grid pattern as `Settings` with filter, `Select visible`, and `Clear all`, instead of the old single-line dropdown chooser.
- Changed: The FastAPI Market Data `Best 1m` form for archive-backed exchanges now uses the shared Backtest editor coin selector together with the same column-based enabled-coins picker used in `Settings`, including filter, `Select visible`, and `Clear all` actions while keeping empty selection = all enabled coins.
- Changed: Hyperliquid `Best 1m` and `Download l2Books` in the FastAPI Market Data page now open as flattened single-purpose content views without the extra outer header card, nested window styling, or expander chrome.
- Changed: The FastAPI Market Data sidebar no longer shows a separate `Actions` section. Its direct shortcuts now stay inside FastAPI as well: `Best 1m` opens a native FastAPI panel for every exchange, and on Hyperliquid the `Best 1m` and `Download l2Books` shortcuts now open focused single-purpose panels instead of showing both actions together.
- Fixed: Latest-1m status tables now hide stale coin rows that are no longer in the current enabled-coins set. The FastAPI monitor filters them immediately, and PBData also drops them from the persisted status at the next cycle start, so removed symbols such as `OM` no longer linger in the fetch list.
- Changed: The FastAPI Market Data TradFi mappings card no longer shows the redundant `New Mapping` header button or the explanatory stock-perp sentence; the workflow stays focused on the table and row actions.
- Changed: The FastAPI Market Data TradFi action button `Spec` is now labeled `Refresh spec`, making it clear that it refreshes the XYZ specification cache.
- Fixed: Market Data TradFi Pyth links now preserve the required `%2F` encoding inside the symbol path and also repair older cached broken URLs at render time, so links like `Equity.US.AMZN%2FUSD` no longer open a 404 page.
- Fixed: Hyperliquid market-data budgeting now allows a full 4-day `candle_snapshot` reservation for latest-1m catch-up runs, fixing a token-bucket burst mismatch that could previously force repeated `budget_timeout` failures even though the API request itself was valid.
- Fixed: The FastAPI Market Data `Search ticker` action now asks for an editable Tiingo query before sending the request, matching the Streamlit workflow instead of always searching only by the raw XYZ symbol.
- Fixed: Market Data TradFi now parses the live XYZ specification columns correctly and derives provider handling from spec-based instrument types, so symbols like `BRENTOIL` classify as `commodity -> no_provider` instead of falling through to `not_found`.
- Fixed: TradFi Auto-Map no longer leaks the last raw JSON symbol into the `Not found` details, and repeated `auto-map: not found` note fragments are now deduplicated so pending rows like `BRENTOIL` stay readable.
- Fixed: The Market Data TradFi Auto-Map summary now uses the same non-delisted merged row scope as the visible symbol mapping table, so hidden delisted leftovers such as `EWZ` no longer inflate `Skipped` counts or appear in Auto-Map details.
- Changed: The Market Data FastAPI TradFi action result box is closable again, and Auto-Map results now expose expandable category details so `Not found`, `Skipped`, and the other summary buckets can be inspected directly.
- Changed: The Market Data FastAPI TradFi mapping editor is now hidden by default and only opens after explicit `New Mapping` or `Edit` actions, instead of staying permanently visible below the table.
- Fixed: The Market Data FastAPI page no longer opens with an empty content area after the sidebar cleanup; the broken `refreshStatuses()` init path was repaired so the active panel is restored and rendered again.
- Fixed: The Market Data FastAPI page no longer leaks raw CSS into the document when opened; the page head/style wrapper and the Help overlay CSS block were restored so the page renders normally again.
- Changed: The Market Data FastAPI sidebar is now navigation-only: the `Overview` entry, the sidebar `Open Legacy Market Data` / `Refresh Status` buttons, and the three informational sidebar cards were removed so only the real page sections and contextual `Settings` actions remain.
- Changed: `View specs` in the Market Data FastAPI TradFi tools now opens the XYZ specs cache in a floating PBGui-style window that can be moved, resized, and closed instead of rendering inline inside the settings page.
- Fixed: Hyperliquid latest `1m` auto-refresh now uses the same per-exchange budget lock as the other Hyperliquid pollers, preventing new or catch-up coins like `XYZ:BIRD` from repeatedly ending in `budget_timeout` before their candle refresh can run.
- Changed: The parallel Market Data FastAPI sidebar now renders the lower `Settings` action buttons in a softer blue secondary palette, making the main navigation buttons and the settings-only functions easier to distinguish at a glance.
- Changed: The parallel Market Data FastAPI sidebar now follows the same pattern as Backtest again: fixed actions stay at the top, the main Market Data sections sit in the middle, and the current `Settings` sub-actions appear only below a separate divider at the bottom.
- Changed: The parallel Market Data FastAPI page now groups the settings sidebar actions more clearly, renames the subsection switches to `Coin Refresh`, `AWS / l2Book`, and `TradFi / Tiingo`, and opens the local `Market Data` guide overlay directly from the shared `Guide` button without navigating away from the page.
- Changed: The parallel Market Data FastAPI page now splits the oversized `Settings` view into sidebar subsections under the `Settings` button, so `Normal Coins Fetching`, `AWS Settings`, and `TradFi Settings` can be opened independently while `Save Settings` stays in the same sidebar context block.
- Fixed: The parallel Market Data FastAPI page no longer forces Hyperliquid `Enabled Coins` to the bottom of the settings stack via a stale flexbox `order` rule, so the card now follows the intended position above `AWS Settings (l2Book)`.
- Fixed: On the parallel Market Data FastAPI page, Hyperliquid `Enabled Coins` now sits directly between the shared top refresh settings and the Hyperliquid `AWS Settings (l2Book)` block, matching the intended layout instead of staying near the end of the Hyperliquid settings stack.
- Changed: The parallel Market Data FastAPI page now moves Hyperliquid `Enabled Coins` out of the shared top settings slot, forces the inline `Status Monitor` to stretch across the full content width, and restores the missing legacy TradFi mapping table fields including `HL Price`, `Tiingo Price`, `Pyth`, `Tiingo Symbol`, `Fetch Start`, and `Verified`.
- Changed: The Hyperliquid `TradFi Symbol Mappings` block on the parallel Market Data FastAPI page now restores the missing legacy actions (`Search ticker`, `Edit`, `Test Resolve`, `Fetch start date`, `Spec`, `Auto-Map`, `Fetch all start dates`, `Refresh metadata`, `Refresh prices`, `View specs`) and uses a wider editor grid so the mapping fields no longer overlap on large screens.
- Changed: The contextual `Save Settings` action on the parallel Market Data FastAPI page now stays in the sidebar directly below `Settings` and only highlights when unsaved setting changes are pending.
- Changed: The Hyperliquid settings order on the parallel Market Data FastAPI page was tightened again so the fetch controls are followed directly by `Enabled Coins`, then the AWS/l2Book settings, and the Tiingo/TradFi mapping blocks remain at the end.
- Changed: The parallel Market Data FastAPI page now mounts `Status Monitor` inline without the nested iframe shell, lets the status table use the remaining page height, moves `Save Settings` into the sidebar while `Settings` is active, and adds a first Hyperliquid `TradFi Symbol Mappings` editor under the reordered Tiingo/AWS settings.
- Added: A parallel `Market Data (FastAPI)` menu entry now opens a new standalone FastAPI shell page with shared PBGui navigation and a live status overview, so the migration can continue without replacing the legacy Market Data page yet.
- Changed: The parallel `Market Data (FastAPI)` shell now uses the shared PBGui sidebar layout for internal navigation instead of top-of-page section switching, matching the other standalone FastAPI pages.
- Added: The parallel `Market Data (FastAPI)` page now includes a first real FastAPI settings slice for Hyperliquid, Binance USDM, and Bybit, reusing the same Market Data config sources and Hyperliquid Tiingo/AWS settings workflow as the legacy page.
- Changed: The `Enabled Coins` picker on the parallel `Market Data (FastAPI)` settings page now uses a searchable checkbox list with selected-coin chips and visible-result counts instead of the raw browser multi-select.
- Changed: The `Enabled Coins` picker was simplified again by removing the duplicate `Selected Coins` block and turning the missing-coin hint into a plain conditional note, so the settings panel stays focused on the single usable selection list.
- Changed: The left `Enabled Coins` card now drops the redundant exchange subheading and lets the coin list grow into the previously unused lower area, so the available space is used for selection instead of empty filler.
- Changed: The settings layout now prioritizes the actual configuration panels above the `Enabled Coins` picker again, with the coin list moved below and capped to a practical height so the page stays usable while still exposing the full selection list.
- Changed: The `Enabled Coins` list now keeps a stepped responsive column flow (`3 -> 2 -> 1`) instead of jumping straight from three columns to one, and the sidebar stacks to the top only at narrower widths.
- Changed: The `Enabled Coins` grid now expands up to 8 columns on very wide screens and steps down gradually (`8 -> 7 -> 6 -> 5 -> 4 -> 3 -> 2 -> 1`) so large browser widths are used efficiently.
- Changed: The Market Data settings eye-toggle now actually reveals and re-hides the AWS and Tiingo password fields, and redundant Auto-Refresh/Tiingo subheadings plus helper text were removed from the panel.
- Changed: The Market Data settings header was trimmed by removing the introductory text, moving the exchange selector to the left, and placing the enabled-coins filter plus actions on one row without redundant `Filter` or `Available Coins` labels.
- Changed: The Market Data settings page now drops the redundant outer settings frame, removes the inner Enabled Coins scroll box, and lets the coin grid continue directly below the filter row so the remaining page height is used without the list sliding into the filter area.
- Changed: Market Data settings feedback now uses transient toast notifications that are also written to the shared Notifications log, instead of leaving success or error messages stuck inline in the panel.
- Added: Market Data settings now include an `Automatically enable all current and new coins` option; when enabled, the refresh workers and related Hyperliquid heatmap/download helpers use the live exchange coin list automatically, and stale removed coins are pruned from manual selections during worker cycles.
- Changed: The parallel Market Data FastAPI page now uses workbench-style sidebar panels for `Status Monitor`, `Actions`, `Already Have`, and `Activity Log`; the standalone `Status Monitor` section mounts the same exchange-specific Market Data status monitor used on the legacy page, while `Actions` keeps the exchange-filtered jobs monitor below the remaining legacy action bridge.
- Changed: The shared Market Data Status Monitor no longer uses a collapsible expander header and now stays open directly in both the standalone FastAPI page and the legacy embedding.
- Changed: The shared Market Data Status Monitor was simplified further by removing its inner title/status header (`Market Data Status Monitor`, `Connected`, `Running`) and the extra top-right refresh button on the standalone status panel.
- Changed: The standalone Market Data page now uses one shared exchange selector at the top of the page for `Settings`, `Status Monitor`, `Actions`, and `Already Have`, instead of repeating separate exchange selectors inside those panels.
