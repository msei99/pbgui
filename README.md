...
- Fixed: Backtest V7 label help markers now underline only the actual wrapped text instead of drawing dotted lines wider than the line content, and row labels reserve a consistent two-line height so neighboring inputs stay horizontally aligned when only one field name wraps
- Fixed: Backtest V7 long single-token labels with underscores now break inside the grid again on narrow widths, so checkbox captions like `filter_by_min_effective_cost` no longer render underneath the next field when the browser gets smaller
- Fixed: Backtest V7 form labels and checkbox captions wrap again on narrow browser widths instead of being clipped away, restoring the earlier responsive behavior for long option names like `filter_by_min_effective_cost`
- Fixed: Backtest V7 custom calendar picker now uses fully custom dark month/year dropdowns instead of browser-native white select popups, and an open picker repositions itself when the browser viewport is resized so it stays visible instead of drifting off-screen
- Fixed: Backtest V7 custom calendar picker now uses dark native month/year control styling instead of white dropdown popups where the browser supports `color-scheme`, and it visibly disables out-of-range days/navigation so `end_date` cannot select dates before `start_date` and `start_date` cannot select dates after `end_date`
- Fixed: Backtest V7 now enforces ordered date ranges in the editor and on save: `end_date` is auto-corrected when a main or suite date picker change would move it before `start_date`, and save/queue is blocked if Raw JSON still contains an invalid backtest or suite scenario date range
- Fixed: Backtest V7 keeps Suite Mode edit state only for Raw JSON reparsing, while opening or reloading the editor normally still resets suite inline editors cleanly; this prevents stale carry-over between configs while stopping Raw JSON scenario edits from collapsing the currently open suite scenario panel
- Fixed: Backtest V7 Raw JSON sync now keeps an open Suite Mode scenario editor open while parsed changes are applied, so editing a scenario in Raw JSON no longer collapses the inline scenario form or causes the Raw JSON viewport to jump because the suite section suddenly closes
- Improved: Backtest V7 Suite Mode scenario editor now uses the same custom calendar picker for `start_date` and `end_date` as the main editor, and `coin_sources` now uses the same exchange-aware chip selector workflow as the main `coin_sources` editor instead of a plain text field
- Improved: Backtest V7 Suite Mode now uses main-editor-style multiselects for scenario `coins` and `ignored_coins`, and the inline suite override/aggregate controls use the dark PBGui input styling instead of browser-default white fields
- Fixed: Backtest V7 editor import now loads pasted JSON directly into the editor for review, valid Raw JSON edits update the structured form in place, and the remaining structured editor actions now write back into Raw JSON too, including `n_positions` steppers, start/end date calendar picks, Suite Mode actions, Coin Overrides actions, and the PBGui data-path helper, without the editor stealing focus, auto-resizing the Raw JSON field during typing, or jumping while async coin reloads finish
- Fixed: CoinData now normalizes HIP-3 coin aliases like `xyz:TSLA`, `XYZ:TSLA`, and `XYZ-TSLA/USDC:USDC` to the canonical `XYZ-TSLA` form so Run/Backtest V7 coin selections keep stock-perp entries selectable and consistent
- Simplified: Run/Backtest V7 editors — removed the separate CoinData `unavailable` coin state; selected coins now resolve as either valid or red invalid entries for the active exchange selection
- Improved: Backtest V7 editor — clarified the inline bulk invalid-removal action so every button consistently reads "Remove all invalid" instead of showing misleading per-row counts
- Improved: Backtest V7 editor — moved the bulk "Remove all invalid" action from the Coins & Filters header into the inline invalid summaries below the affected coin lists
- Improved: Backtest V7 editor — added a single "Remove invalid" action in Coins & Filters that removes all currently invalid approved/ignored coin entries across long and short lists at once
- Fixed: Backtest V7 editor — opening an invalid config now surfaces the real passivbot validation error in PBGui instead of failing with a generic JSON parse/Internal Server Error message
- Improved: Run/Backtest V7 editors — `approved_coins` and `ignored_coins` now resolve through CoinData to short names in the multiselect UI, invalid entries stay visible as red tags, and Backtest now builds its coin option list from the union of all selected non-`combined` exchanges
- Fixed: PBRun — local V7 bots now self-heal stale `running_version.txt` values while running and rewrite the correct version after successful restarts; this prevents falsely showing `outdated` with `run_ver = 0` for active local bots
- Fixed: Log Viewer — local instance logs (e.g. `bybit_SANDUSDT/passivbot.log`) now appear in the FILES sidebar when running locally; previously only daemon logs (PBRun, PBRemote, PBCoinData) were listed; instance entries shown with 🤖 prefix
- Fixed: Run/Backtest log panels — bumped shared `log_viewer_panel.js` asset version so browsers load the updated local-instance log handling instead of using a stale cached module
- Fixed: Run V7 log panel — when an instance is enabled on the local master host (e.g. `magicnucpro`), the Log button now opens the viewer in local file mode instead of misclassifying it as a remote VPS service view
- Fixed: Run V7 log panel — local Restart on bot logs now targets the real `RunV7` process for the selected instance; removed the broken `pid.txt` fallback assumption so PBRun can relaunch the bot after kill
- Improved: Backtest V7 editor — config form layout reorganized: Rows 2 & 4 merged into single 8-column row (all span 1: `starting_balance`, `balance_sample_divider`, `btc_collateral_cap`, `btc_collateral_ltv_cap`, `max_warmup_minutes`, `minimum_coin_age_days`, `liquidation_threshold`, `dynamic_wel_by_tradability`); `market_order_slippage_pct` reduced from span 2 to span 1 in Row 3; Coins & Filters section now matches run editor: `market_cap`/`vol/mcap` at span 2 each, `tags` at span 2 (was span 4)
- Fixed: Run V7 editor — coin override files (COIN.json) from a backtest config are now copied to the new run instance on first save when using "▶ Add to Run"; only files actually referenced in `coin_overrides` are copied (previously all .json files including stale USDT-named variants were copied)
- Improved: Run V7 editor — added tooltips (mouse-over help) to all remaining Advanced Settings fields: `forced_mode_short`, `time_in_force`, `max_n_cancellations_per_batch`, `max_n_creations_per_batch`, `max_n_restarts_per_day`, `recv_window_ms`, `order_match_tolerance_pct`, `max_concurrent_api_requests`, `balance_hysteresis_snap_pct`, `max_warmup_minutes`, `warmup_jitter_seconds`, `warmup_concurrency`, `memory_snapshot_interval_minutes`, `volume_refresh_info_threshold_seconds`, `max_disk_candles_per_symbol_per_tf`, `max_memory_candles_per_symbol`, `inactive_coin_candle_ttl_minutes`, `enable_archive_candle_fetch`, `max_ohlcv_fetches_per_minute`
- Improved: Backtest Configs — search input is focused automatically when switching to the Configs panel
- Fixed: Run V7 editor — "Add to Run" now always resets `enabled_on` to `disabled` in the draft; previously the backtest config's VPS name (e.g. `lion`) caused a blank "Enabled on" select when that host doesn't exist in the run editor's host list
- Fixed: Run V7 editor — "Enabled on" always writes `disabled` instead of empty string when no host is selected; added `disabled` as first option in the host dropdown
- Improved: Backtest Configs — added "▶ Add to Run" sidebar button when a config is open (same user-conflict modal as Results)
- Improved: Backtest Configs — added name search filter above the config list
- Fixed: Backtest Results/Archive filter now searches only the backtest name, not exchange or config name
- Fixed: Run V7 editor — Coin Overrides panel now looks identical to Backtest editor: added missing `.tbl`, `.act-btn`, `.cov-badge`, `.cov-tt-*`, `#cov-tooltip` CSS classes and tooltip hover JS
- Improved: Run V7 editor — upper form section (rows 1–3) and Filters section converted to single responsive 8-column grids (same pattern as Advanced Settings); User/Enabled-on widen to span 2 each; Note widens to span 4; market_cap/vol_mcap/Tags each span 2 columns
- Improved: Run V7 editor — Advanced Settings expander reorganized from 6 rows (cols-4) into 4 thematic rows (cols-8): Modes & Policies, Order Execution, Balance/Warmup/Logging, Candle Data; added `hsl_signal_mode` and `hsl_position_during_cooldown_policy` dropdowns
- Improved: Run V7 editor — added 3 new `live.*` parameters as dedicated GUI fields: `margin_mode_preference` (Row 1, next to Leverage), `max_realized_loss_pct` (Row 2, risk gate), `market_order_near_touch_threshold` (Row 3, companion to market_orders_allowed) via `GET /configs/new-config` instead of an empty object; backtest, bot, live, and optimize sections are pre-filled with the installed passivbot version's defaults
- Improved: Run V7 editor — new instance now loads passivbot schema defaults via `GET /instances/new-config` (always in sync with installed passivbot version); old hardcoded minimal defaults removed
- Improved: Run V7 editor — "Additional Parameters" expander auto-renders any `live.*` keys not yet handled by the GUI; "Raw JSON" expander shows/edits the full config (same pattern as Backtest editor)
- Improved: Run V7 instance list — edit button moved to right-side actions column (together with delete); rows can now be opened by double-click
- Improved: Run V7 editor — Coin Overrides now use the shared `coin_overrides_editor.js` module (same as Backtest editor); inline ~350-line implementation removed; per-coin config files saved via new `GET/PUT /api/v7/override-config/{name}/{filename}` endpoints and new `GET /api/v7/override-params` endpoint; `param_status` highlighting (amber=neutralized, red=pb_default) added to Long/Short bot JSON editors
- Fixed: PBRemote — `alive()` heartbeat was inside the `for server in remote_servers` loop; on fresh deployments with an empty bucket the loop never executes, so no heartbeat was ever sent (chicken-and-egg deadlock between master and slave). Moved `alive()` one level up so it always runs
- Fixed: `vps-init.yml` — changed `debug: var:` to `debug: msg:` in Play 3 debug tasks; newer Ansible (2.18+) rejects `lookup()` inside `var:` as untrusted template expression, and `var:` semantics were wrong anyway (expects a variable name, not a Jinja expression)
- Improved: Run V7 editor — config load/save now uses `pb7_config` pipeline (same as Backtest editor); old `ConfigV7` dependency removed; renamed params (e.g. `filter_*` → `forager_*`) are migrated automatically; newly injected params are neutralized to safe defaults and highlighted in the Full Config JSON editor
- Fixed: `pb7_config` — `forager_score_weights` neutral value corrected to `{"ema_readiness": 1.0, "volume": 0.0, "volatility": 0.0}` so pb7's normalized-weights validation passes when volume/volatility scoring is disabled
- Fixed: Login page — relay from FastAPI back to Streamlit no longer flashes the login page without a password prompt; `st.stop()` is now called immediately after `st.switch_page()` in the relay block
- Fixed: V7 Backtest — navigating from any Streamlit menu item to "Backtest" now redirects directly to the FastAPI page instead of loading the old Streamlit backtest UI
- Improved: V7 Backtest config editor — date fields (`start_date`, `end_date`) now show a visible 📅 button that opens the native calendar picker; removed dependency on the tiny browser-native calendar icon
- Improved: V7 Results & Archive — liquidated backtests are now visually highlighted: red ⚠ icon before the name and subtle red row background (e.g. `combined/`, `config_name/`) up to the archive root, so no orphaned folders remain
- Improved: V7 Archive — local archives directory is now watched via Linux inotify; when files are created, deleted, or moved (e.g. by a background `git pull`), all connected clients receive an `archive_update` WebSocket message and refresh automatically without polling; archive list and results view are now lazy-loaded once and only refreshed on inotify events
- Fixed: V7 Archive — switching to the Archive panel now always reloads the archive list; previously it only loaded once (when empty), so newly added configs were not visible until page refresh
- Fixed: V7 Archive — switching back to an already-open archive results view now fetches fresh data instead of re-rendering cached results; "Add to Archive" also immediately refreshes the results if the same archive is currently open
- Improved: pb7_config — when loading a result or saved config for editing, parameters silently injected by passivbot's pipeline are now detected via `_transform_log` and highlighted inline in the Full Config JSON editor; neutralized params (amber background) and unknown new params (red background, "review") are color-coded directly on the affected JSON lines; no separate info panel
- Fixed: V7 Backtest Configs — result count no longer stays 0 after a backtest completes; the configs table now auto-refreshes via WebSocket when a running job transitions to `complete`
- Fixed: V7 Backtest Results — "Add to Archive" now uses the correct destination name (`config_name/timestamp`) instead of only the timestamp; matches Streamlit behavior (split on `/pbgui/`)
- Fixed: V7 Backtest Results — "Add to Archive" now uses the configured own archive (`my_archive` from INI) directly, like Streamlit; removed the unnecessary archive picker dropdown
- Fixed: V7 Backtest Results — "Optimize from Result" was missing the relay token in the redirect URL, causing the login page to appear instead of the Optimize editor
- Fixed: V7 Backtest Results — "🧬 Optimize from Result" now navigates to the V7 Optimize editor with the result config pre-loaded as `starting_config`; previously landed on the empty config list
- Fixed: V7 Backtest Results — `optimizeFromResult()` function declaration was accidentally omitted, causing a top-level `return` syntax error that broke the entire page JS
- Fixed: V7 Backtest Results — "⚡ Calc Balance" and "💰 Balance Calculator" are now both present in the sidebar (same as editor sidebar): Calc Balance shows inline modal with exchange selector + auto-calculation; Balance Calculator navigates to standalone page with config pre-loaded
- Improved: V7 Backtest Results — "⚡ Calc Balance" sidebar button: "🔍 Strategy Explorer", "⚡ Calc Balance", "🧬 Optimize from Result", "🗄 Add to Archive"; "Add to Archive" shows an archive picker modal and calls the existing archive API
- Improved: V7 Backtest Configs — results count cell is now clickable (opens Results filtered to that config); ✏ Edit button restored in actions; double-click row opens editor; 📊 View Results button removed (redundant)
- Improved: V7 Backtest Configs — removed Edit (✏) and Duplicate (📄) action buttons from the configs table; double-click a row to edit, save under a different name to duplicate
- Improved: V7 Backtest — Guide button now opens an inline help overlay (same as API Keys / VPS Monitor pages) pre-selected on the PBv7 Backtest topic with EN/DE language toggle and full-text search; no longer navigates to the Streamlit help page
- Improved: V7 Backtest — Help guides (EN + DE) fully rewritten to reflect the FastAPI UI (panels, Settings modal, HLCVS cleanup, error restart button, floating log panel)
- New: V7 Backtest — automatic HLCVS cache cleanup; removes old `pb7/caches/hlcvs_data` directories to free disk space; configurable retention (default 7 days) and check interval (default 24h) in Settings; "🧹 Clean Now" button for immediate manual cleanup with freed-space feedback
- Improved: V7 Backtest — settings (CPU, Autostart, HLCVS cleanup) moved from inline sidebar to a dedicated Settings modal dialog; sidebar now has a single "⚙ Settings" button
- Improved: All FastAPI pages — "🔔" button in the top nav bar opens an inline floating notification log panel (`PBV7UI.log`) on every page; no new tabs needed; central `POST /api/notify_log` endpoint in `PBApiServer.py`; toast messages from `v7_backtest`, `v7_run`, `v7_edit`, and `api_keys_editor` are written there with timestamp + level
- Improved: V7 Backtest Queue — rows are now mouse-selectable (click to toggle, drag to multi-select) like Results and Archive; added "Select All" / "Deselect" toolbar buttons and "🗑 Delete Selected" button in the sidebar
- Fixed: V7 Backtest — "View Results" button in Queue always fetches fresh results from API before filtering; previously used stale in-memory cache so a just-completed backtest was not visible until manual refresh
- Improved: V7 Backtest — "Backtest" multi-select modal now includes "📂 Use PBGui Market Data" checkbox; when checked, `ohlcv_source_dir` is set to the PBGui data path before queuing (works for both Results and Archive panels)
- Fixed: V7 Backtest Archive — `config_name` now read from `backtest.base_dir` last path segment (e.g. `RENDER_adg_sharpe_omega_twe9_trgr_eema125`) instead of constructing a slash-containing string from directory parts; avoids `/` in queue names
- Fixed: V7 Backtest Queue — jobs with `error` status now show a ▶ Restart button that immediately relaunches the backtest (independent of Autostart setting)
- Fixed: V7 Backtest Archive — navigating away from archive results and back reset sidebar to list actions (`ctx-archive`) and overwrote `_activeResultsCtx.chartsEl` with `results-charts`; `selectPanel('archive')` now detects open archive results via `selectedArchiveName`, restores `ctx-archive-results` sidebar and re-renders results table to restore correct chart context
- Fixed: V7 Backtest — backtest names showed double `//` for archive results and contained only `config_name` without exchange/timestamp context; both API endpoints now return `display_name` as full relative path from their base dir (e.g. `v7.3.13/config/bybit/2026-01-27...`), used in table and all chart titles
- Fixed: V7 Backtest — charts sometimes rendered empty on second open: cache-hit renders called `Plotly.newPlot` synchronously before browser layout pass, so Plotly measured zero width; fixed by deferring cache-hit renders via `requestAnimationFrame`
- Fixed: V7 Backtest — 📌 Pin/Unpin: when unpinned, panel now gets `overflow:visible; flex:none; height:auto` to release all clipping constraints so `#main-content` scroll activates correctly; was broken for both Results and Archive (`.view-panel.active { overflow:hidden }` was silently clipping content even in unpinned mode)
- Improved: V7 Backtest Archive — archive results sidebar now shows 🏠 Archives (back), 🔄 Backtest, ▶ Add to Run, 📈 Compare, 🗑 Delete Selected — identical to Results tab; archive list sidebar shows only git actions (Pull All, Push, Setup, Log)
- Improved: V7 Backtest Archive — results view now switches to a full-screen layout identical to the Results tab (🏠 Archives home button, sticky table with drag-resize handle, charts scroll below); archive list is hidden while viewing results, restored on "Archives" back button
- Improved: V7 Backtest Archive — results view now uses the same full table+chart view as the Results tab (sortable columns, icon buttons 👁🖼📋📄🔬); table rendering extracted into shared `_renderResultsTableInto()` helper; archive has independent sort state
- Improved: V7 Backtest Archive — "⬇ Pull All" now pulls all archives at once (like Streamlit git_pull); output shown in modal; git operations logged to `data/logs/ArchiveSync.log`
- Improved: V7 Backtest Archive — Git Push now does a `git pull` before commit+push (like Streamlit), and uses a timestamped commit message; push output shown in modal
- Improved: V7 Backtest Archive — configurable auto-pull interval (minutes, 0 = off) in ⚙ Setup; background `ArchiveSyncWorker` pulls all archives automatically at the configured interval
- Fixed: V7 Backtest Archive — Log panel (Archive Sync Log) was not draggable/resizable; extracted shared `_openLogPanel()` helper so drag and resize handlers are always bound regardless of which log is opened first
- Improved: V7 Backtest Archive — archive rows open results on double-click; removed redundant 📊 View button (Delete button remains); `JSON.stringify(name)` produced double-quoted strings that broke the `onclick` HTML attribute; fixed to use `esc(name)` with escaped single-quotes (matching the Configs/Queue pattern)
- Improved: V7 Backtest Archive — added ⚙ Setup dialog to configure own archive (archive selection, path, username, email, access token) with Test-Push (dry-run) and Save; Git Push now uses stored credentials and injects access token into HTTPS remote URL for authentication (matching Streamlit behaviour)
- Improved: V7 Backtest Config Editor — "Additional Parameters" expander auto-renders any `backtest.*` keys not yet handled by the GUI (detected at load time); values are type-aware (bool → checkbox, number → number input, JSON → textarea, string/null → text field); new keys are preserved on save; future passivbot parameters become immediately usable without PBGui changes
- Improved: V7 Backtest Coin Overrides — summary table now shows a compact badge (e.g. "long 12 · short 3") instead of a long parameter name list; hovering the badge shows a tooltip table with all parameter names and their values
- Fixed: Ansible playbooks (`master-update-pb.yml`, `master-update-pbonly.yml`, `master-switch-pb7-branch.yml`, `master-pb7-python312.yml`) — after maturin builds the Rust extension, a stamp-writing step now updates the `.rust-src-sha256` fingerprint file; previously ansible bypassed `rust_utils.recompile_rust()` so the stamp was never updated, causing passivbot to falsely detect the extension as stale on the next startup and recompile unnecessarily
- Fixed: V7 Backtest Queue — page jitter caused by full table re-render every 3 seconds on WebSocket updates; now skips re-render when queue data is unchanged
- Improved: V7 Backtest Results — liquidation detection now uses passivbot's native `liquidated` flag from analysis.json when available; falls back to multi-signal heuristic (drawdown_worst >= 0.95, equity_balance_diff_neg_max >= 0.95, or final_balance below liquidation_threshold) for older results without the flag; detail view warning shows which criteria triggered
- Fixed: V7 Backtest — override config files (COIN.json) were not used at backtest time: queue was copying the config to `pb7/configs/backtest/` instead of using the original `data/bt_v7/{name}/backtest.json` directly (as the Streamlit version does), so override files next to it couldn't be found; queue now points to the original config file; also ensured override files always contain `live: {}` for passivbot flavor detection
- Fixed: Dashboard — chart zoom no longer resets when new income data arrives via WebSocket; PNL, ADG, PPL, and Income charts now preserve the user's zoom state across live data updates
- Fixed: Dashboard — Income list (table mode) scroll position no longer jumps back to top on WebSocket data updates; scroll position is preserved across re-renders
- Improved: V7 Backtest Coin Overrides — coin names are now auto-normalized on load (e.g. `HYPEUSDT` → `HYPE`, `1000BONKUSDT` → `BONK`); duplicates after normalization are merged (short name wins); override files are renamed to match normalized names on save; orphaned files cleaned up
- Fixed: V7 Backtest — "Save As" with normalized coin names now correctly copies override files from source (e.g. `HYPEUSDT.json` → `HYPE.json`); previously the copy failed silently because it looked for the normalized filename which didn't exist in the source directory
- Fixed: V7 Backtest — loading override config files now works with normalized coin names; API falls back to finding the pre-normalization file on disk (e.g. requests `BONK.json` but finds `1000BONKUSDT.json`)
- Improved: V7 Backtest Coin Overrides — pasting a complete passivbot config into the Config File textareas now auto-filters to only allowed override parameters; detects structured configs (`bot.long`/`bot.short`) and extracts the relevant side; also filters on load so existing files with extra params are shown reduced and saved correctly
- Improved: V7 Backtest Config Editor — Coin Overrides now use the same two-tier model as passivbot and old Streamlit: "Config File" section saves `COIN.json` directly to disk (bot.long/bot.short overrides), "Inline Overrides" (parameter chips) for quick tweaks like forced_mode/leverage that take precedence over the file; `override_config_path` is managed automatically; new `PUT /override-config` API endpoint for saving override files; preserves passivbot merge order (base → file → inline)
- Fixed: V7 Backtest — "Save As" (saving with a new name) and Duplicate now copy `override_config_path` files (e.g. `1000BONKUSDT.json`) to the new config directory; previously only `backtest.json` was copied, leaving broken file references
- Fixed: V7 Backtest — table headers in Configs, Queue, and Archive truly stay fixed at top while scrolling: each panel is now a flex column with its own scroll container for the table area, so headers never scroll off and content no longer leaks above them
- Fixed: V7 Backtest Results — panel appeared empty when switching to it if results had been pre-loaded by the config editor sidebar; now always renders on panel switch
- Added: V7 Backtest Config Editor — Coin Overrides editor: per-coin parameter overrides (bot.long, bot.short, live) with inline editing, searchable parameter picker and value input from passivbot allowlist via new `GET /override-params` API endpoint; searchable coin picker (same as approved_coins); supports `override_config_path` for file-based overrides; collapsible Config section with long/short JSON textareas pre-filled with base config (like Streamlit) — overrides merged on top, only changed values stored as overrides on save; `override_config_path` files are loaded via new `GET /override-config/{name}/{file}` API and merged into Config view
- Fixed: V7 Backtest Config Editor — Coin Overrides parameter picker input had white background (`.form-group input` CSS overrode `.ms-input` transparent style) and Add button showed "Select a parameter first" despite selection (added input-value fallback)
- Fixed: V7 Backtest Config Editor — Raw JSON expander now auto-expands textarea to full content height when opened (previously stuck at fixed 20 rows)
- Removed: V7 Backtest Queue — redundant Refresh button (queue already receives live updates via WebSocket)
- Improved: V7 Backtest Coin Overrides — override config files (COIN.json) are now loaded through passivbot's `prepare_config` pipeline for proper normalization and migration; saved as sparse diffs (only override params) via raw `json.dump`; UI textareas show only allowed override params for display/editing
- Fixed: V7 Backtest API — added missing `import sys` in `api/backtest_v7.py`
- Improved: V7 Backtest Results — Config and Analysis JSON panels now auto-expand to show the full content without scrolling; Collapse button still available to shrink back
- Improved: V7 Backtest Queue — double-click on a queue item’s name opens the config editor for that config
- Improved: V7 Backtest log panel — now resizable from all 4 edges and 4 corners (custom drag handles replace CSS `resize: both` which only worked from bottom-right)
- Fixed: V7 Backtest Compare chart — removed "Compare" title that overlapped with the legend
- Fixed: V7 Backtest Suite — Exchange Comparison template now auto-adds missing exchanges to the base config; previously scenarios with an exchange not in the base config had zero coins and failed with `has no coins after applying exchange filters`
- Fixed: V7 Backtest Results — clicking Backtest on a suite result now uses the correct config name (`config_name` from API) instead of parsing the path, which incorrectly picked the scenario label for suite results
- Fixed: V7 Backtest sidebar — CPU stepper buttons and Settings section had no styling (missing `.sb-stepper` CSS); added proper stepper styles to shared `sidebar.css`, removed inline styles
- Fixed: V7 Backtest Config Editor — saving with a changed name no longer deletes the original config (copy instead of rename); `backtest.base_dir` is now set to `backtests/pbgui/{name}` on every save so results land in the correct directory; suite override keys use `bot.long.xxx` / `bot.short.xxx` prefix for correct passivbot application
- Improved: V7 Backtest Queue — completed items now show a 📊 button that jumps directly to the Results panel filtered by config name
- Added: V7 Backtest Config Editor — Suite Mode editor (`frontend/js/suite_editor.js`): enable/disable suite mode, manage multiple scenarios with labels, date ranges, exchange subsets, coin filters, and bot parameter overrides; built-in templates (Exchange Comparison, Date Windows, TWE Sensitivity, n_positions Sensitivity); collapsible aggregate settings with per-metric method overrides; new `GET /bot-params` API endpoint loads override parameters dynamically from passivbot schema
- Improved: V7 Backtest Config Editor — approved_coins now supports `"all"` per side; loading handles all passivbot formats (`"all"` string, per-side object, old `empty_means_all_approved` migration); `★ all` chip shown in green; exclusive toggle in dropdown
- Improved: V7 Backtest Config Editor — config_name field is now editable; saving with a changed name renames the config (old folder is removed)
- Removed: V7 Backtest Config Editor — `empty_means_all_approved` field removed (deprecated in passivbot, auto-migrated)
- Improved: V7 Backtest Config Editor — added help tooltips to Coins & Filters fields (market_cap, vol/mcap, tags, only_cpt, notices_ignore); updated EN and DE help docs with a Coins & Filters parameter table
- Improved: V7 Backtest Config Editor — coin_sources/market_settings_sources chip editors: selecting a coin already added with a different exchange offers a swap (shows `COIN @ old_exchange ⇄`, updates on click); selecting same exchange removes it; × all clears all entries; keyboard Enter handles swap too
- Improved: V7 Backtest Config Editor — coin_sources and market_settings_sources replaced table+checkbox UI with compact chip-based editor: exchange dropdown loads available coins, searchable coin picker, one-click × removal; entries shown as `exchange:COIN` chips
- Improved: V7 Backtest Config Editor — Bot Configuration: TWE and n_positions sync bidirectionally with the Full Config JSON textarea; textarea auto-expands to show full JSON without scrolling
- Improved: V7 Backtest Config Editor — Coins & Filters row now cols-8 (market_cap(1), vol/mcap(1), tags(4), only_cpt(1), notices_ignore(1)); checkboxes side-by-side instead of stacked
- Improved: V7 Backtest Config Editor — reordered rows (Identity, Balance, Fees, Behavior, Data Source); all checkboxes bottom-aligned; `dynamic_ignore` removed (live-only); `empty_means_all_approved` moved above approved coins; Apply Filters moved from checkbox to sidebar button
- Improved: V7 Backtest Config Editor — logically grouped fields into 5 rows (Identity & Time, Balance & Collateral, Data Source, Backtest Behavior, Fees & Overrides); consistent cols-8 grid, boolean flags distributed into their thematic rows
- Improved: V7 Backtest Config Editor — added help tooltips (dotted underline, hover) for all fields from passivbot's configuration.md; labels normalized to snake_case
- Improved: V7 Backtest Config Editor — added missing fields: `liquidation_threshold`, `market_order_slippage_pct`, `dynamic_wel_by_tradability`
- Improved: V7 Backtest Config API — migrated from ConfigV7 property classes to passivbot's native config pipeline (`load_prepared_config`), providing automatic schema migration, missing-field hydration, and legacy format support; PBGui-specific fields preserved separately
