...
- Sidebar pages: always use the shared `frontend/css/sidebar.css` structure (`#sidebar`, `#sidebar-sticky`, `#sidebar-header`, `#sidebar-toolbar`, `#sidebar-inner`, `#sidebar-resize`) and `frontend/js/sidebar_resize.js`. Do not recreate the base sidebar layout, controls, or resize behavior in page-local CSS/JavaScript; page-local rules may only style the sidebar's domain-specific content.
...
### Security Boundaries
- Never place passwords, private keys, PBGui authentication/session tokens, or other secrets in PBGui-controlled URLs/query strings, page source, browser storage, command lines, or logs. If a third-party provider requires a server-side query token, keep that URL out of browsers and logs. Send unsaved secrets to PBGui in authenticated request bodies and redact diagnostic payloads.
- Exception: A stored third-party API key or API token may be revealed in the browser only after an authenticated user explicitly clicks the eye/reveal control for that specific credential. The value must be requested from a dedicated same-origin authenticated endpoint using POST with the credential ID in the request body, returned with `Cache-Control: no-store`, and assigned only to the corresponding password input's `value` property. It must not appear in URLs, initial page payloads, rendered HTML source, `innerHTML`, logs, diagnostics, analytics, browser storage, clipboard operations, or bulk credential responses. The frontend must clear the revealed value when it is hidden, when another credential is selected, when the page loses authentication, and when the page is unloaded. PBGui passwords, private keys, session tokens, authentication tokens, and credential-store encryption material must never be revealable.
...
This project is indexed by GitNexus as **pbgui** (22089 symbols, 123344 relationships, 1555 execution flows).
