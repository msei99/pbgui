...
- Fixed the Streamlit About menu version text to read the current `PBGUI_VERSION` directly from `pbgui_purefunc.py`, and bumped the API serial so the UI shows that a restart is needed before a still-running process can pick up the fix.
