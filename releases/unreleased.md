...
- VPS Manager: require per-host VPS user passwords during Import Cluster Nodes so the import can refresh remote settings, install monitoring SSH keys and keep passwords only in the current session instead of leaving hosts half-imported.
...
- VPS Manager: added an Import Cluster Nodes action that previews and imports safe metadata from Cluster Sync nodes without copying passwords, CoinMarketCap keys or private keys.
