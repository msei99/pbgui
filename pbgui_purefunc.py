...
def get_syncable_files(instance_name: str) -> list[tuple[str, bytes]]:
    """Return (filename, content) tuples for syncable config files.

    Includes config.json + coin configs. Excludes runtime files.
    """
    inst_dir = PBGDIR / "data" / "run_v7" / instance_name
    if not inst_dir.is_dir():
        return []
    files = []
    for f in sorted(inst_dir.iterdir()):
        if f.is_file() and f.suffix == ".json" and f.name not in SYNC_EXCLUDE_FILES:
            try:
