...
<main>
  <section>
    ...
    <p style="color:var(--muted)">Install a remote PBGui master VPS from this local browser wizard. OpenVPN and TOTP are mandatory.</p>
  </section>
  <section class="panel">
    <h2>Remote Master VPS</h2>
    <p class="recommendation">Recommended VPS: <a href="https://aklam.io/CBA3zSaZ" target="_blank" rel="noopener noreferrer">IONOS VPS Linux M+</a> with 4 vCores CPU, 4 GB RAM, and 120 GB NVMe.</p>
    <form id="install-form" class="grid">
      <label>Initial login mode
      ...
      <label>VPS IP or hostname <input name="remote_host" required placeholder="1.2.3.4"></label>
      <label>SSH username <input name="ssh_username" id="ssh-username" value="root"></label>
      <label>SSH password
      ...
      <label id="root-password-wrap">New root password (optional)
        <span class="password-wrap">
      ...
      <label>Target PBGui user <input name="target_user" id="target-user" value="%%TARGET_USER%%"></label>
      <label id="target-password-wrap">Target user password
        <span class="password-wrap">
      ...
      <label>Remote hostname / OpenVPN profile name <input name="hostname" value="pbgui-master"></label>
      <label>Swap size
      ...
      <label id="swap-custom-wrap" style="display:none">Custom swap size <input id="swap-custom" placeholder="10G"></label>
      ...
      <label>PBGui bind address <input name="pbgui_bind_host" value="0.0.0.0"></label>
      <label>PBGui port <input name="pbgui_port" type="number" value="8000" min="1024" max="65535"></label>
      <label>OpenVPN network CIDR <input name="openvpn_cidr" value="10.8.0.0/24" placeholder="10.8.0.0/24"></label>
      <label>SSH firewall mode
      ...
      <label class="full" id="ssh-ips-wrap">Allowed SSH source IPs, comma-separated <input name="ssh_allowed_ips" id="ssh-allowed-ips" placeholder="detecting your public IP..."></label>
      <div class="warning full" id="ssh-warning">
...
sshMode.addEventListener('change', syncMode); loginMode.addEventListener('change', syncLogin); sshUsername.addEventListener('input', () => { if (loginMode.value === 'sudo') targetUser.value = sshUsername.value; }); swapSizeSelect.addEventListener('change', syncSwapSize); swapCustom.addEventListener('input', syncSwapSize);
syncMode(); syncLogin();
syncSwapSize();
...
function renderResult(job) {
  ...
  resultEl.innerHTML = (job.status === 'done'
...
form.addEventListener('submit', ev => {
  ev.preventDefault();
  if (sshMode.value === 'anywhere' && !sshRisk.checked) {
    resultEl.style.display = 'grid';
...
""".replace("%%TARGET_USER%%", html.escape(target_user, quote=True)).replace("%%TARGET_USER_JSON%%", json.dumps(target_user))
