...
def select_instance():
    ...
    st.data_editor(data=d, height=36+(len(d))*35, key=f"editor_select_instance_{st.session_state.ed_key}", hide_index=None, column_order=None, column_config=column_config, disabled=['id','User','Symbol','Market_type','Enabled On','Version','Remote','Remote Version'])
    st.subheader("Instances used in Multi configuration")
    st.data_editor(data=d_multi, height=36+(len(d_multi))*35, key="editor_select_instance_multi", hide_index=None, column_order=None, column_config=column_config, disabled=['id','User','Symbol','Market_type','Enabled On'])
