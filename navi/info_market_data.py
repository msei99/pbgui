        ...
        if str(exchange).lower() == "binance":
            ...
            with st.expander("Build best 1m OHLCV (Binance USDM)", expanded=False):
                ...
                def _bnc_best_jobs_fragment():
                    ...
                    render_fastapi_job_monitor(height=600, exchange="binanceusdm", job_type="binance_best_1m")
            ...
            with st.expander("Build best 1m OHLCV (Bybit)", expanded=False):
                ...
                def _bybit_best_jobs_fragment():
                    from pbgui_func import render_fastapi_job_monitor
                    render_fastapi_job_monitor(height=600, exchange="bybit", job_type="bybit_best_1m")
            ...
            # Download l2Book + Build OHLCV (FastAPI + Vanilla JS — localStorage-persisted collapsible sections)
            @st.fragment
            def _hl_data_actions_fragment():
                from pbgui_func import render_fastapi_hl_data_actions
                render_fastapi_hl_data_actions()
            _hl_data_actions_fragment()
                                    ...
                                    while cur_day <= dt1:
                                        ...
                                        if 0 <= doy_idx < max_days:
                                            ...
                                            if is_stock_perp_1m:
                                                ...
                                                if is_holiday_ov:
                                                    exp_ov = 0
                                                if exp_ov == 0 and api_v == 0 and l2b_v == 0 and oth_v == 0:
                                                    if is_holiday_ov:
