...
def pbdata_details():
    ...
    # WS payload debug toggle persisted in pbgui.ini under [pbdata] debug_ws
    try:
        current = load_ini('pbdata', 'debug_ws')
        cur_bool = False
        if isinstance(current, str) and current.lower() in ['1', 'true', 'yes', 'on']:
            cur_bool = True
    except Exception:
        cur_bool = False

    debug_checkbox = st.checkbox('WS Payload Debug Logging', value=cur_bool, key='pbdata_debug_ws_checkbox')
    if 'pbdata_debug_ws_checkbox' in st.session_state and st.session_state['pbdata_debug_ws_checkbox'] != cur_bool:
        save_ini('pbdata', 'debug_ws', 'true' if st.session_state['pbdata_debug_ws_checkbox'] else 'false')
        st.success('Saved debug_ws setting to pbgui.ini')
        st.rerun()
        ...
        if summary_path.exists():
            ...
            if summary_obj:
                ...
                try:
                    ...
                    # Render as columns with metrics for clearer visibility
                    c_bal, c_pos, c_ord, c_hist = st.columns(4)
                    # Use simple markdown with colored emojis for readability (no raw HTML)
                    with c_bal:
                ...
                rows = []
                # Last-fetch timestamps (epoch seconds) per user from summary JSON
                ...
                with st.expander("Details", expanded=False):
                    ...
                    # Apply filter
                    def matches(row):
                        # rows now contain combined display values like 'ws (2m)' or 'rest (never)'
                    ...
                    if filtered:
                        # Helper: determine color for minutes text
                        def color_for_minutes(mins_text):
                            try:
                                ...
                                if mins_text.endswith('m'):
                                    ...
                                    n = int(mins_text)
                                # Color mapping (recent -> green, mid -> light green, old -> orange/red)
                                if n <= 5:
                                    return '#2ecc71'  # green
                                if n <= 60:
                                    return '#9be7a3'  # light green
                                if n <= 240:
                        ...
                        def render_cell(display_str):
                            # display_str expected like 'ws (2m)' or 'rest (never)'
                            try:
