        ...
        if summary_path.exists():
            ...
            if summary_obj:
                ...
                for u in users:
                    rows.append({
                        'user': u,
                        'balances': 'ws' if u in (bal.get('ws') or []) else 'rest',
                        'positions': 'ws' if u in (pos.get('ws') or []) else 'rest',
                        'orders': 'ws' if u in (ords.get('ws') or []) else 'rest',
                        'history': 'rest' if u in (hist or []) else ''
                    })
                ...
                with st.expander("Details", expanded=False):
                    ...
                    # Apply filter
                    def matches(row):
                        if show_bal_ws and row['balances'] != 'ws':
                            return False
                        if show_pos_ws and row['positions'] != 'ws':
                            return False
                        if show_ord_ws and row['orders'] != 'ws':
                            return False
                        return True
